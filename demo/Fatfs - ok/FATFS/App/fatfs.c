...
// 全局时间结构体
FatFs_Time_t g_fatfs_time = {2026, 3, 20, 0, 0, 0};
...
{
    // 边界保护，防止时间越界
    if (g_fatfs_time.year < 1980) g_fatfs_time.year = 1980;
    if (g_fatfs_time.month < 1)  g_fatfs_time.month = 1;
    if (g_fatfs_time.month > 12) g_fatfs_time.month = 12;
    if (g_fatfs_time.day < 1)    g_fatfs_time.day = 1;
    if (g_fatfs_time.day > 31)   g_fatfs_time.day = 31;

    return ((DWORD)(g_fatfs_time.year - 1980) << 25)
         | ((DWORD)g_fatfs_time.month << 21)
         | ((DWORD)g_fatfs_time.day << 16)
         | ((DWORD)g_fatfs_time.hour << 11)
         | ((DWORD)g_fatfs_time.minute << 5)
         | ((DWORD)(g_fatfs_time.second / 2));
}
...
void FatFs_ListDir(void)
{
    ...
    while (1)
    {
        ...
        {
            UsbPrintf("%-12s | %04d-%02d-%02d %02d:%02d:%02d | %lu KB /%10lu 字节\r\n",
