...
void AT_FATFS_CluWrite_Handle(char* command)
{
    ...
    uint16_t head_len = strlen(cmd_header);

    if (strSearch(command, cmd_header) != 0)
    {
        UsbPrintf("[command] 错误：格式错误\r\n");
        UsbPrintf("[command] 用法：AT^FATFS_CluWrite=<簇号>,<文件名>,<数据>\r\n");
        return;
    }

    char* p = command + head_len;
    char* comma1 = strchr(p, ',');
    char* comma2 = strchr(comma1 + 1, ',');

    if (!comma1 || !comma2)
    {
        UsbPrintf("[command] 错误：参数格式错误\r\n");
        return;
    }

    char cluster_buf[16] = {0};
    strncpy(cluster_buf, p, comma1 - p);
    DWORD cluster = atol(cluster_buf);

    char filename[64] = {0};
    strncpy(filename, comma1 + 1, comma2 - (comma1 + 1));
    ...
    {
        UsbPrintf("[command] 簇%lu 越界 → 自动使用簇2\r\n", cluster);
        cluster = 2;
    }

    UsbPrintf("\r\n******[command] 写入开始******\r\n");
    UsbPrintf("[command] 簇号：%lu\r\n", cluster);
    UsbPrintf("[command] 文件名：%s\r\n", filename);
    UsbPrintf("[command] 数据：%s\r\n", data);
    UsbPrintf("[command] 长度：%d\r\n", data_len);

    File_Write_Result_t wr;
    FRESULT res = FatFs_FileWrite_Cluster(filename, cluster, data, data_len, &wr);

    if (res == FR_OK && wr.write_len > 0)
    {
        UsbPrintf("[command] 写入成功！字节：%u\r\n", wr.write_len);
        UsbPrintf("[command] 起始簇：%lu  当前簇：%lu\r\n",
            wr.start_cluster, wr.curr_cluster);
    }
    else
    {
        UsbPrintf("[command] 写入失败！错误码：%d\r\n", res);
    }

    UsbPrintf("******[command] 结束******\r\n");
}
