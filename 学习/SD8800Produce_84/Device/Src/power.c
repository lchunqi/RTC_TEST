...
void rtc_vertical_open_one(uint8_t left)//left: 0 ~ 13
{
    HAL_GPIO_WritePin(GPIOD, GPIO_PIN_0 << left, GPIO_PIN_RESET);
}

void rtc_vertical_close_one(uint8_t left)//left: 0 ~ 13
{
...
void rtc_vertical_control(uint8_t lineNum, uint8_t mode)
{
    uint8_t i = 0;
    if (mode == ALL_CLOSE) {
        HAL_GPIO_WritePin(GPIOD, 0x3fff, GPIO_PIN_SET);
    } else if (mode == ALL_OPEN) {
        HAL_GPIO_WritePin(GPIOD, 0x3fff, GPIO_PIN_RESET);
...
void rtc_horizontal_bat_control(uint8_t lineNum, uint8_t mode)
{
    ...
    if (mode == ALL_CLOSE) {
        ...
        HAL_GPIO_WritePin(GPIOB, GPIO_PIN_2|GPIO_PIN_3|GPIO_PIN_4|GPIO_PIN_5|GPIO_PIN_6|GPIO_PIN_7, GPIO_PIN_RESET);
    } else if (mode == ONE_OPEN_OTHER_CLOSE) {
...
void rtc_group_power(uint8_t channel, uint8_t mode)
{
    ...
    switch (mode) {
        ...
        case POWER_BAT_I:    //BAT电流测试模式
            ...
            HAL_GPIO_WritePin(GPIOE, GPIO_PIN_7, GPIO_PIN_RESET);//INT 拉低
            //选择行开关
            rtc_horizontal_bat_control((channel - 1) / 14, ONE_OPEN_OTHER_CLOSE);
            rtc_horizontal_vdd_control(0, ALL_CLOSE);//VDD通路关闭
            //选择列开关
            rtc_vertical_control((channel - 1) % 14, ONE_OPEN_OTHER_CLOSE);
            HAL_GPIO_WritePin(GPIOB, GPIO_PIN_8 | GPIO_PIN_9 | GPIO_PIN_12 | GPIO_PIN_13 | GPIO_PIN_14 | GPIO_PIN_15, GPIO_PIN_SET);//VCE全部变为低电平
            HAL_GPIO_WritePin(GPIOC, 0x03ff, GPIO_PIN_RESET);//SCL 拉低
        break;
        case POWER_VDD_I:    //VDD电流测试模式
            ...
            HAL_GPIO_WritePin(GPIOE, GPIO_PIN_7, GPIO_PIN_RESET);//INT 拉低
            //选择行开关
            rtc_horizontal_vdd_control((channel - 1) / 14, ONE_OPEN_OTHER_CLOSE);
