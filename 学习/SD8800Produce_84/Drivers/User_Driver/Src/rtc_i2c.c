...
void rtc_set_channel(uint8_t channel, RtcI2cState state)
{
	...
	switch (state) {
		...
		break;
		case RTC_INIT_1006:
			sda_channel = ((channel - 1) / 14);
			g_i2c.busSdaIn = GPIOE;
			g_i2c.pinSdaIn = GPIO_PIN_0 << sda_channel;
			...
			soft_rtc_delay = 600;
  			HAL_GPIO_WritePin(GPIOB, GPIO_PIN_8|GPIO_PIN_9|GPIO_PIN_12|GPIO_PIN_13|GPIO_PIN_14|GPIO_PIN_15, GPIO_PIN_SET);//必须，所有6行同时导通会导致下拉电阻过小，分压不足
		...
		case RTC_SD3031:
			...
			soft_rtc_delay = 50;
			rtc_scl_init(GPIO_MODE_OUTPUT_OD);
  			HAL_GPIO_WritePin(GPIOB, GPIO_PIN_8|GPIO_PIN_9|GPIO_PIN_12|GPIO_PIN_13|GPIO_PIN_14|GPIO_PIN_15, GPIO_PIN_SET);//必须,拉高7号线，便于操作4号线
			if (sda_channel <= 1) {
