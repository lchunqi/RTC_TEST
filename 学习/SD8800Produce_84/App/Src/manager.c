...
//step 1: read system temp ship, and check system voltage ok
    ...
    //     UsbPrintf("check system voltage is error, test stop.\n");
...
void system_set_int(uint8_t channel)
{
    ...
    // if (lastChannel == -1) {//启动的初始状态
    // //可能存在瞬间断电重启的可能，此时RTC可能没掉电,寄存器数值还保留
...
void system_manage_test_mode(void)
{
    ...
    switch (g_testMode) {
                ...
                case 2:
                    if (g_capAdjust.flag != CAP_ADJUST_DEFAULT) {//只要状态改变，就进行下一个芯片测试，不管是否成功
                        ...
                        if (g_capAdjust.flag == CAP_ADJUST_SUCCESS) {//先进行1次数据校验，若校验不合格就不保存
                            // if (check_rtc_temp_success(count, g_oneData.number, g_capAdjust.temp) > 0) {
                            ...
                            // } else {
                            //     UsbPrintf("check rtc%d temp fail\n", count);
        ...
        break;
        case SYNC_RTC_TIME:
            system_get_real_time(&sysTime);
            count = 0;
            for (i = 0; i < MAX_CHIP_NUMBER; i++) {
                rtc_group_power(i + 1, POWER_I2C);
                if (rtc_write_date(&sysTime) == I2C_OK) {
                    count++;
                    log_printf("%d sync success\n", i + 1);
                    UsbPrintf("%d sync success\n", i + 1);
                } else {
                    log_printf("%d sync i2c error\n", i + 1);
                    UsbPrintf("%d sync i2c error\n", i + 1);
                }
            }
			ControlPrintf("okNumber:%d chipName:%d message:同步时间完成\n", count, MAX_CHIP_NUMBER);
            log_printf("sync end\n");
            UsbPrintf("sync end\n");
            g_testMode = FREE_MODE;
        break;
        case CHECK_ERROR_TIME:
            ...
            for (i = 0; i < MAX_CHIP_NUMBER; i++) {
                rtc_group_power(i + 1, POWER_I2C);
                if (rtc_read_date(&rtcTime) == I2C_OK) {
                    result = system_tool_calculate_sec(sysTime) - system_tool_calculate_sec(rtcTime);
                    if (-5 <= result && result <= 5) {
                        count++;
                    } 
                    log_printf("chip%d time:%d, all number:%d\n", i + 1, result, count);
                    UsbPrintf("chip%d time:%d, all number:%d\n", i + 1, result, count);
                } else {
                    log_printf("%d i2c error\n", i + 1);
                    UsbPrintf("%d i2c error\n", i + 1);
                }
            }
			ControlPrintf("okNumber:%d chipName:%d message:校验时间完成\n", count, MAX_CHIP_NUMBER);
            log_printf("check end\n");
            UsbPrintf("check end\n");
            g_testMode = FREE_MODE;
        break;
        case MACHINE_TEST://仅测试，不写入
            if (count == 0) {
                ...
                result = system_machine_test_one(count, 0);
                if (result != 0) {//不为0才表示测试结束
                    // ControlPrintf("chip %d is %d", count, result);
                    if (result > 0) {
        ...
        case MACHINE_COVER://先清空，测试后写入
            ...
            g_testMode = MACHINE_WRITE;
            count = 0;
        break;
        case MACHINE_WRITE://测试后写入
            if (count == 0 || count > MAX_CHIP_NUMBER) {
                count = 1;
                system_machine_write_one(count, 1);
            } else if (count <= MAX_CHIP_NUMBER) {
                result = system_machine_write_one(count, 0);
                if (result != 0) {//不为0才表示测试结束
                    // ControlPrintf("chip:%d is %d", count, result);
                    if (result > 0) {
                        g_testFlag[count - 1] = '1';
                        ...
                        g_testFlag[count - 1] = 0xff;//相当于不写入数据
                        ControlPrintf("chipName:%d message:fail\n", count);
                    }
                    if (count < MAX_CHIP_NUMBER) {
                        count++;
