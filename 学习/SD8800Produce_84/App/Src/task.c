...
RealTime g_realTime = {24, 5, 6, 9, 0, 0, 0};
...
void Aecq100Sample(void)//临时任务 ACEQ100测试需要5000个芯片
{
    ...
    switch (Aecq100State) {
        ...
        break;
        case 7:
            if (sd8800_ACEQ100(config, (uint16_t)(rtc_temp - (temp_read_value(1) - 25) * 2), lastCap + 2) > 0) {
                UsbPrintf("download sd8800 success\n");
                ControlPrintf("download:烧写成功\n");
                Aecq100State ++;
                lastTestTime = HAL_GetTick();
                UsbPrintf("sd8800 power off\n");
                rtc_group_power(AECQ100_CHANNEL, POWER_OFF);
            } else {
                ControlPrintf("download:烧写失败\n");
                UsbPrintf("download sd8800 fail\n");
                Aecq100State = 50;
			    HAL_Delay(5);
                ControlPrintf("result:NG\n");
            } 
