			...
			if (g_DeviceRxBuff[i] == '[') {
				for (j = i; j < length; j++) {
					if (g_DeviceRxBuff[j] == '0' || g_DeviceRxBuff[j] == '1' || g_DeviceRxBuff[j] == 'N') {
						if (getNumber == 84) {//数量超范围了，肯定是错误数据	
							DevicePrintf("ERROR\n");
							return; 
						}
...
void AT_1006TMODE_Handle(char* command)
{
	...
	if (mode > 0) {
		...
		if (wave1006_enter_test_mode(WAVE1006_TEST_PAD | WAVE1006_TEST_OSC | WAVE1006_TEST_ADC | WAVE1006_TEST_HOT_BG) != WAVE1006_EXIT_OK)   //此处一定要注意，没进入测试模式也需要将I0切换掉，或者不能继续运行程序
		{
			return;
		}
		UsbPrintf("pad change OK\n");
		HAL_Delay(5);
		rtc_set_channel(g_rtcChanenel, RTC_SD3031);
		if (drv_wave1006_read_reg(DEV_SD3068, WAVE1006_REG_USER_RAM_START + 19, 1, &buf) != WAVE1006_EXIT_OK) 
		{
			return;
		}
		buf &= ~0x28;   //清0 RuleTempAdj[2:0]      //ADC尺子 暂选5
		buf |=  0x28;
		...
		rtc_otp_power(0);
		rtc_set_channel(g_rtcChanenel, RTC_INIT_1006);
		UsbPrintf("RTC exit testmode success\n");
	}
}
