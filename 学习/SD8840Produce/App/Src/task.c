...
void system_manage(void)
{
    ...
    if (systemTick % 50 == 0 && g_intState) {//临时命令
            command_int_test(1);
    }
    if (systemTick % 50 == 0 && g_aecq100Start) {//临时命令
            Aecq100Sample();
    }
...
void command_int_test(uint8_t init)
{
    ...
    float ppm;
    float temp;
	uint8_t config[5] = {0x01, 0x00, 0x11, 0x00, 0x10};

    if (init == 0) {
        testStep = 0;
        buf = 0x00;
        spi_write_multi_bytes(0x0f, &buf, 1);//关闭之前开启频率输出
        return;
    }
    ...
    switch (testStep) {
        ...
        case 1:
			...
			rtc_set_channel(testNumber, RTC_SD8840);
            testStep++;
        break;
        case 2:
			buf = 0x10;
...
void command_cap_list_test(uint8_t init, int startNumber)
{
    ...
    static int capNumber = 0;

    if (init == 1) {
        g_intState = 1;
        testStep = 0;
        capNumber = startNumber - 1;
        return;
    } else if (init == 0) {
        g_intState = 0;
        return;
    ...
    switch (testStep) {
        ...
        case 1:
            ...
            osc_buf[1] = (mode << 3) | (capNumber / 256);
            osc_buf[0] = capNumber % 256;
        ...
        case 3:
            if (g_lastFreq.recordTime > lastTick) {
                ...
                if (capNumber == 0) {
                    ...
                    if ((lastPPM + 30) < ppm || (lastPPM - 30) > ppm) {
                        lastPPM = ppm;
                        capNumber--;
                        testStep = 1;
                        break;
    ...
    switch (testState) {
        ...
        case 3:
            if (g_lastFreq.recordTime > lastTestTime) {
                ...
                // if (fabs(temp_read_value(1) - 22) < 50) {             
                    if (0 < ppm && ppm < 1) {
                        lastCap += 1;
                    } else if (-1 < ppm && ppm < 0) {
                        lastCap -= 1;
...
//-----------------------------测试频率------------------------------------------
        ...
        case 5:
            if (g_lastFreq.recordTime > timeout) {//有新的数据更新，计算误差
                ...
                ppm4096 = (g_machineTest.machine.testFreq / CENTER_FREQENCY - 1) * 1000000;
...
//-------------------------------
// void Aecq100Sample(void)//临时任务 ACEQ100测试需要5000个芯片
// {
//     uint8_t buffer[8];
// 	float temp, ppm;
...
//     static float minCap;
//     static uint8_t adjustCount = 0;
// 	uint8_t setting[5] = {0x01, 0x00, 0x11, 0x00, 0x10};
//     float current;
//     int i;
//     // uint8_t result;
...
// 			rtc_set_channel(AECQ100_CHANNEL, RTC_SD8840);
//             HAL_Delay(5);
// 			spi_read_multi_bytes(0x00, buffer, 8);
//             for (i = 0; i < 8; i++) {
...
//             if (i < 8) {
//                 ClearControlMessage();
//                 ControlPrintf("chip:SD8840\n");
//                 HAL_Delay(5);
//                 ControlPrintf("download:此前烧写成功\n");
//                 Aecq100State = 9;
//                 lastTestTime = HAL_GetTick();
//                 HAL_Delay(5);
//                 ControlPrintf("id:8840不能读ID\n");
//             } else{
...
//             //   if (rtc_8840_adc_testmode(AECQ100_CHANNEL) == I2C_OK) {  
...
//         break;
//         case 2:
//             if (HAL_GetTick() > lastTestTime + 500) {
//                 temp = temp_read_value(1);
...
//                 if (config > 50 && config < 130) {//温度偏移值正常
//                     // rtc_1006_exit_testmode(AECQ100_CHANNEL); //先退出ADC测试模式
//                     // rtc_1006_set_testmode(AECQ100_CHANNEL);//再进调电容模式
//                     rtc_out_4096();
//                     Aecq100State++;
//                 } else {
...
//         break;
//         case 7:
//             if (sd8840_ACEQ100(config, (uint16_t)(rtc_temp - (temp_read_value(1) - 25) * 2), lastCap + 2) > 0) {
//                 UsbPrintf("download sd8840 success\n");
//                 ControlPrintf("download:烧写成功\n");
//                 Aecq100State ++;
//                 lastTestTime = HAL_GetTick();
//                 UsbPrintf("sd8840 power off\n");
//                 rtc_group_power(AECQ100_CHANNEL, POWER_OFF);
//             } else {
//                 ControlPrintf("download:烧写失败\n");
//                 UsbPrintf("download sd8840 fail\n");
//                 Aecq100State = 50;
// 			    HAL_Delay(5);
//                 ControlPrintf("result:NG\n");
//             } 
...
//                 rtc_set_channel(AECQ100_CHANNEL, RTC_SD8840);
//                 buffer[0] = 0x10;
//                 spi_write_multi_bytes(0x57, buffer, 1);
//                 Aecq100State++;
//                 break;
//             }
//         break;
...
// 			spi_write_multi_bytes(0x57, buffer, 1);
// 			HAL_Delay(10);
// 			spi_read_multi_bytes(0x56, buffer, 1);
...
// 			spi_write_multi_bytes(0x0B, setting, 5);//开启输出
//             lastTestTime = HAL_GetTick();
//             Aecq100State++;
//             break;
//         case 11:
...
//                 ppm = (g_lastFreq.freq / CENTER_FREQENCY - 1) * 1000000;
//                 UsbPrintf(" %fHz, %.2fppm\n", g_lastFreq.freq, ppm);
//                 ControlPrintf("freq:%fHz\n", g_lastFreq.freq);
// 			    HAL_Delay(5);
//                 ControlPrintf("ppm:%.2fppm\n", ppm);
//                 buffer[0] = 0x00;
//                 spi_write_multi_bytes(0x0f, buffer, 1);//关闭之前开启频率输出
//                 if (ppm > 3 || ppm < -3) {
//                     ControlPrintf("result:NG\n");
//                     Aecq100State = 50;
//                 } else {
...
void Aecq100Sample(void)//临时任务 ACEQ100测试需要5000个芯片
{
    ...
    switch (Aecq100State) {
        case 0:
            ...
            if (i < 8) {
                ...
                (buffer[0] != 0 || buffer[1] != 0 || buffer[2] != 0 || buffer[3] != 0 || buffer[4] != 0 || buffer[5] != 0)) {
                    ...
                    ControlPrintf("chip:SD8840\n");
                    HAL_Delay(5);
                    ControlPrintf("download:未烧写，通信OK\n");
                    HAL_Delay(5);
        ...
        case 10:
			...
			UsbPrintf("temp:%.0f, system temp = %.1fC\n", rtc_temp, temp);
            ControlPrintf("chipTemp:%.0f\n", rtc_temp);
            if (rtc_temp > (3 + temp) || rtc_temp < (-3 + temp)) {
                HAL_Delay(10);
                ControlPrintf("result:NG\n");
                Aecq100State = 50;
                break;
        ...
        case 12:
            if (g_lastFreq.recordTime > lastTestTime) {
                ppm = (g_lastFreq.freq / CENTER_FREQENCY - 1) * 1000000;
                UsbPrintf(" %fHz, %.2fppm\n", g_lastFreq.freq, ppm);
                if (ppm > (CENTER_FREQ + 2.5) || ppm < (CENTER_FREQ - 2.5)) {
                    lastTestTime = HAL_GetTick();
                    g_lastFreq.recordTime = lastTestTime;
                    DevicePrintf(":MEAS?\n");
                    Aecq100State++;
        ...
        case  50:
            if (rtc_result > 0) {                                   //等待芯片拿开
		        ...
		        rtc_set_channel(AECQ100_CHANNEL, RTC_INIT_1006);
                HAL_Delay(5);
                if (rtc_read_multi_bytes(0x64, 0x00, buffer, 3) != I2C_OK ) {
                    Aecq100State = 0;
                }
            }
            if (HAL_GetTick() > lastTestTime + 1000) {
