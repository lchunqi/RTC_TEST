...
{
	UsbPrintf("soft version: V1.1\n");
	UsbPrintf("hard version: V1.0\n");
...
void AT_RTCOTP_Handle(char* command)
{
	...
	int inNumber = 0;
	CharToInt(command, strSearch(command, "=") + 1, &inNumber);
	if (inNumber <= 0) {
		HAL_GPIO_WritePin(GPIOE, GPIO_PIN_6, GPIO_PIN_RESET); //关闭8.5v的开关
	} else {
...
void AT_WRITECONFIG_Handle(char* command)//暂时调试使用20231109
{
	// uint8_t i2c;
	// uint8_t buffer = 0x66;
	// rtc_group_power(1, POWER_DOWNLOAD);
	// HAL_Delay(100);
	// i2c = rtc_download_otp_data(&buffer, 0x82, 1);
	// // i2c = rtc_write_otp_config();
	// if (i2c == I2C_OK) {
	// 	UsbPrintf("OK\n");
	// } else {
	// 	UsbPrintf("i2c error\n");
...
{	
	...
	if (number > 0) {
		CharToInt(command, strSearch(command, ",") + 1, &number);
		command_cap_list_test(1, number);
		UsbPrintf("cap list test start...\n");
	} else {
		command_cap_list_test(0, 0);
		UsbPrintf("cap list test end...\n");
	// UsbPrintf("OK\n");
	}
	UsbPrintf("OK\n");
}
...
void AT_1006TMODE_Handle(char* command)
{
	...
	if (mode > 0) {
		...
		HAL_Delay(5);
		rtc_set_channel(g_rtcChanenel, RTC_SD3302);
...
void AT_RTCLOAD_Handle(char* command)
{
	...
	CharToInt(command, strSearch(command, "=") + 1, &number);
	if (number > 0) {
		sd8840_write_otp(number);
	} else {
		for (i = 1; i <= MAX_CHIP_NUMBER; i++) {
			result[i - 1] = sd8840_write_otp(i);
		}
...
void AT_TEMPADC_Handle(char* command)
{
	...
	// rtc_set_channel(1, RTC_SD8840);
	// g_rtcChanenel = 1;
	// HAL_Delay(10);
	// spi_read_multi_bytes(0x00, buf, 8);
	// UsbPrintf("read group[%d] reg[0x%x]:", 1, 0);
	// for (i = 0; i < 8; i++) {
	// 	UsbPrintf("0x%02x ", buf[i]);
	// }
	UsbPrintf("\nOK\n");
}

extern uint8_t Aecq100State;
extern uint8_t g_aecq100Start;
void AT_AECQ100_Handle(char* command)
{       
	int number = 0;
	CharToInt(command, strSearch(command, "=") + 1, &number);
	g_aecq100Start = number;
	if (number > 0) {
