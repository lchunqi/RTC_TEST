...
wave1006_otp_arg_t g_sd8840_config = {
	0,         		//otp_wp_config
	1,         		//test_wp_config
	DEV_SD8840,		//device_type_config 
	0,         		//hour_12_config          		// no support 12 hour
	1,         		//week_encode_config   	  		//00-06 as sunday-monday
	3,         		//freq_en_config    	    		//use one bit enbale control
	PKG_SD8840,		//package_type_config
	...
	0,				//vol_sel_config
	5, 				//freq_src_config						//use 4 bit select frequency
...
//	otp_prg_power_ctrl(0); 
 	rtc_set_channel(rtcNumber, RTC_SD3302);//切换pad引脚
	HAL_Delay(10);
    /*vdd=1*/
  buf[0] = 0x80;
...
uint8_t sd8840_calcul_cap(uint8_t channel)
{
	int i;
	for (i = 0; i < 420; i++) { 
		g_cap_val[i] = 0; 
...
uint8_t sd8840_write_otp(uint8_t channel)
{
	int i;
	uint8_t buffer[2] = {0};
	RealTime date = {0};
	...
	if (sd8840_calcul_cap(channel) == 0) { return 0;}
	...
	g_sd8840_config.temp_offset_config = g_offset;			 //温度偏移值，不同芯片不一样
	...
	UsbPrintf("*************** WRITE SD8840 ************\r\n");
	if (wave1006_write_otp(&g_sd8840_config, g_dev_id, g_temp_arg, channel) != WAVE1006_EXIT_OK) {
		return 0;
	}
	return 1;
}

uint8_t sd8840_ACEQ100(uint16_t config, uint16_t rtcAdc, float maxCap)
{
	int i;
	uint8_t buffer[2] = {0};
	RealTime date = {0};
	...
	//烧写带温补的OTP    配置为SD3031   烧写保护位		
	g_sd8840_config.temp_offset_config = config;			 //温度偏移值，不同芯片不一样
	// UsbPrintf("write offset:%d\n", g_sd8840_config.temp_offset_config);
	g_dev_id[0] = ((date.year / 10) % 10) * 16 + (date.year % 10);
	g_dev_id[1] = ((date.mon / 10) % 10) * 16 + (date.mon % 10);
	g_dev_id[2] = ((date.day / 10) % 10) * 16 + (date.day % 10);
	...
	UsbPrintf("*************** WRITE SD8840 ************\r\n");
	if (wave1006_write_otp(&g_sd8840_config, g_dev_id, g_temp_arg, AECQ100_CHANNEL) != WAVE1006_EXIT_OK) {
		return 0;
	}
	return 1;
}
