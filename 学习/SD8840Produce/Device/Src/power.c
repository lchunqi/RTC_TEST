...
void rtc_group_power(uint8_t channel, uint8_t mode)// SD8840没有BAT引脚
{
    ...
    switch (mode) {
        case POWER_OFF:  //全部关闭电源
            HAL_GPIO_WritePin(GPIOB, GPIO_PIN_2 | GPIO_PIN_3 | GPIO_PIN_4 | GPIO_PIN_5, GPIO_PIN_SET);//模拟开关切换到悬空
            HAL_GPIO_WritePin(GPIOE, GPIO_PIN_6, GPIO_PIN_SET);//VCE_SDA选择VCE通道，但VCE均关闭
            HAL_GPIO_WritePin(GPIOB, GPIO_PIN_8 |GPIO_PIN_9, GPIO_PIN_SET);//VCE控制引脚拉高，从而变为低电平
            ...
            HAL_GPIO_WritePin(GPIOE, GPIO_PIN_0 |GPIO_PIN_1 |GPIO_PIN_2 |GPIO_PIN_3 |GPIO_PIN_4 |GPIO_PIN_5 |GPIO_PIN_7, GPIO_PIN_RESET);//INT 拉低
        ...
        case POWER_VDD_I:    //VDD电流测试模式
            ...
            HAL_GPIO_WritePin(GPIOB, GPIO_PIN_8 |GPIO_PIN_9, GPIO_PIN_SET);//VCE控制引脚拉高，从而变为低电平
            HAL_GPIO_WritePin(GPIOC, GPIO_PIN_0 |GPIO_PIN_1 |GPIO_PIN_2 |GPIO_PIN_3 |GPIO_PIN_4 |GPIO_PIN_5 |GPIO_PIN_6 |GPIO_PIN_7, GPIO_PIN_SET);//VCE控制引脚拉高，从而变为低电平
            HAL_GPIO_WritePin(GPIOB, GPIO_PIN_2 | GPIO_PIN_3 | GPIO_PIN_4 | GPIO_PIN_5, GPIO_PIN_SET);//模拟开关切换到悬空,使得MOSI和MISO引脚被下拉电阻接地
        break;
        default:break;
    }
}