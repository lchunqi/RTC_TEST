...
void system_manage_test_mode(void)
{
    ...
    switch (g_testMode) {
        ...
        case MACHINE_COVER://先清空，测试后写入
            ...
            count = 0;
            UsbPrintf("machine test clear!\n");
            ControlPrintf("message:擦除测试记录\n");
        break;
        case MACHINE_WRITE://测试后写入
            if (state == 0) {//初始状态，开启机台测试模式
                state = 1;
                memset(g_testFlag, 0xff, MAX_CHIP_NUMBER);
                flash_test_flag_manage(g_testFlag, FLAG_MACHINE, FLASH_FLAG_READ);
                count = 0;
            } else if (state == 1) {//开启某一个芯片的测试
                count++;          
                if (count <= MAX_CHIP_NUMBER) {          
                    if ( g_testFlag[count - 1] == 0xff) {
            ...
            } else {//等待并判断某一个芯片的测试结果
                result = system_machine_write_one(count, 0);//触发测试
                if (result != 0) {//不为0才表示测试结束
                    if (result > 0) {
                        g_testFlag[count - 1] = '1';
                        number = 0;
                        for (i = 0; i < count; i++) {
                            if (g_testFlag[i] == '1') { number++; }
                        }
                        ControlPrintf("okNumber:%d chipName:%d message:OK\n", number, count);
                    } else {
