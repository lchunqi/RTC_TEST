...
char funtionList[MAX_LIST_NAMEBER][FUNCTION_NAME_LENGTH] = {
    "[管理存储]", "[接触测试]", "[配置温度计]", "[温箱测试]", "[温补计算]", "[温补烧写]", "[老化测试]", "[机台测试]", "[取料记录]"
};
...
void show_machine_message(void)
{
    int i;
    static uint32_t lastTick = 0;
    if (g_rxTick[1] >= lastTick) {
        lastTick = g_rxTick[1];
        for (i = 0; i < 11; i++) {
            SetTextValue(SCREEN_MACHINE_TEST, 3 + i, g_testMessage[i]);
        }
    }
}
...
void lcd_manage(void)
{
    ...
    if (judge_key_effect(KEY_ENTER, KEY_SELECT_LONG)) {
        screenId = SCREEN_FUNCTION_SELECT;
        SetScreen(screenId);
        // listNumber = 1;
    ...
    switch (screenId) {
        case DEFAULT_START:
            ...
            show_screen_real_time(screenId, 1);
        break;
        case SCREEN_FUNCTION_SELECT:
            show_screen_real_time(screenId, 11);
            show_borad_message();
            ...
            if (judge_key_effect(KEY_ENTER, KEY_SELECT_SHORT)) {
                if (listNumber == 4) {
                    screenId = SCREEN_VOLTAGE_SELECT;
                    SetScreen(screenId);
                    function_one_select1(selectNumber);
                } if (listNumber == 8) {
                    screenId = SCREEN_MACHINE_TEST;
                    SetScreen(screenId);
                } else {
                    screenId = SCREEN_MASSAGE_SHOW;
                    SetTextValue(screenId, 1, funtionList[listNumber - 1]);
                }
                SetScreen(screenId);
                functionNumber = 0;
                SetTextValue(SCREEN_MASSAGE_SHOW, FUNCTION_SELECT_ID, selectList[functionNumber]);
        ...
        case SCREEN_VOLTAGE_SELECT:
            ...
            if (judge_key_effect(KEY_ENTER, KEY_SELECT_SHORT)) {
                screenId = SCREEN_MASSAGE_SHOW;
                SetScreen(screenId);
                SetTextValue(screenId, 1, funtionList[3]);
            }
        break;
        case SCREEN_MASSAGE_SHOW:
            show_screen_real_time(screenId, 4);//时间控件是4
        ...
        break;
        case SCREEN_MACHINE_TEST:
            show_screen_real_time(screenId, 2);//时间控件2
            show_machine_message();
        break;
        default:break;
    }
}
...
void StartCommunicateTask(void const * argument)
{
    ...
    while (1) {
        ...
        lcd_manage();
        //------------------------
        CheckControlList();//LCD 接收串口信息，暂时没有使用
    }
}
