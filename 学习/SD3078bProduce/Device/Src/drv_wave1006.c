...
wave1006_otp_arg_t g_sd3068_config = {
	0,         //otp_wp_config
	1,         //test_wp_config
	DEV_SD3068,//device_type_config 
	1,         //hour_12_config          		// no support 12 hour
	0,         //week_encode_config   	  		//00-06 as sunday-monday
	1,         //freq_en_config    	    		//use one bit enbale control
	PKG_SD3078A,//package_type_config
	0,         //ae_polarity_config  	 			//1 is alarm enable
	1,         //week_day_alarm_config 			//both week and day can select alarm
	0,         //wada_polarity_config  			//if the "week_day_alarm config" as 1,this bit not use
	1,         //week_alarm_config     			//every bit represent a day
	0x7f,      //alarm_valid_config    			//second -year can alarm
	0,         //ae_disable_config     			//when all alarm is disable, alarm is close
	0,         //timer_freq_source_config		//use 2 bit select timer frequency source 
	1,				 //id_support_config
	0, 				 //timer_pause_support_config	//not support timer pause
  	0, 				 //timer_reset_support_config	//not support timer reset
	0,				 //int_out_sel_config					//use INTS1:0 bits select alarm or freq output
  	0,				 //int_out_rst_config					//int output high when flag clear or int is disable
  	0, 				 //wp_type_config							//use three wp bit
	0, 				 //time_rst_support_config		//not support time power on reset
	0, 				 //group2_alarm_support_config//not support group2 alarm
	1, 				 //reg_addr_reset_config			//read and write stop addr is reset
	0, 				 //time_update_support_config	//not support time update
	...
	0,				 //digtal_comp_ext_config	    //support digtal extend,digtal_comp_sel_config must this bit has effective
	0,				 //ram_ext_config							//not support ram extend
	...
	5,  			 //rule_temp_adj_config
  	4,				 //rule_bat_adj_config
	2,				 //osc_cap_sel_config1        //封装电容选择值 3pf
	...
	0,				 //vol_sel_config
	0, 				 //freq_src_config						//use 4 bit select frequency
	1, 				 //k32_support_config					// support 32k alone output
...
wave1006_exit_code_e wave1006_write_otp(wave1006_otp_arg_t const *config_arg,  unsigned char const *id, wave1006_temp_arg_t const *temp_arg, uint8_t rtcNumber)
{
	...
	{
		UsbPrintf("write otp step 1 fail!\r\n");
		return WAVE1006_EXIT_FAIL;
	}  
	//选择为SD3302封装(必须在进测试模式之前选择，否则芯片pad改变)
	if (drv_wave1006_read_reg(DEV_SD3068, WAVE1006_REG_USER_RAM_START + 20, 1, &buf1) != WAVE1006_EXIT_OK)   //回读ram21
	{
		UsbPrintf("write otp step 2 fail!\r\n");
		return WAVE1006_EXIT_FAIL;
	}
	buf1 &= ~0x0f;
	buf1 |= 0x0d;
...
uint8_t sd3078b_calcul_cap(uint8_t channel)
{
	int i;
	for (i = 0; i < 420; i++) { 
		g_cap_val[i] = 0; 
...
uint8_t sd3078b_write_otp(uint8_t channel)
{
	int i;
	uint8_t buffer[2] = {0};
	RealTime date = {0};
	...
	if (sd3078b_calcul_cap(channel) == 0) { 
		return 0;
	}
	...
	//烧写带温补的OTP    配置为SD3078   烧写保护位		
	g_sd3068_config.temp_offset_config = g_offset;			 //温度偏移值，不同芯片不一样
	...
	UsbPrintf("*************** WRITE SD3078 ************\r\n");
	if (wave1006_write_otp(&g_sd3068_config, g_dev_id, g_temp_arg, channel) != WAVE1006_EXIT_OK) {
		return 0;
	}
	return 1;
}

uint8_t sd3078b_ACEQ100(uint16_t config, uint16_t rtcAdc, float maxCap)
{
	int i;
	uint8_t buffer[2] = {0};
	RealTime date = {0};
	...
	//烧写带温补的OTP    配置为SD3078   烧写保护位		
	g_sd3068_config.temp_offset_config = config;			 //温度偏移值，不同芯片不一样
	g_dev_id[0] = ((date.year / 10) % 10) * 16 + (date.year % 10);
	g_dev_id[1] = ((date.mon / 10) % 10) * 16 + (date.mon % 10);
	g_dev_id[2] = ((date.day / 10) % 10) * 16 + (date.day % 10);
	...
	UsbPrintf("*************** WRITE SD3078 ************\r\n");
	if (wave1006_write_otp(&g_sd3068_config, g_dev_id, g_temp_arg, AECQ100_CHANNEL) != WAVE1006_EXIT_OK) {
		return 0;
	}
	return 1;
}
