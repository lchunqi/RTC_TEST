...
/*********************************12*12字库***********************************************/
void OLED_ShowChinese12X12(uint8_t X, uint8_t Y, char *Chinese)
{
	uint8_t pChinese = 0;
	uint8_t pIndex;
	uint8_t i;
	...
	for (i = 0; Chinese[i] != '\0'; i ++)		//遍历汉字串
	{
		...
		if (pChinese >= OLED_CHN_CHAR_WIDTH)
		{
			...
			/*将汉字字模库OLED_CF16x16的指定数据以16*16的图像格式显示*/
			OLED_ShowImage(X + ((i + 1) / OLED_CHN_CHAR_WIDTH - 1) * 16, Y, 12, 12, OLED_CF12x12[pIndex].Data2);
		}
	}
}

/*********************************10*10字库***********************************************/
void OLED_ShowChinese10X10(uint8_t X, uint8_t Y, char *Chinese)
{
	uint8_t pChinese = 0;
	uint8_t pIndex;
	uint8_t i;
	...
	for (i = 0; Chinese[i] != '\0'; i ++)		//遍历汉字串
	{
		...
		if (pChinese >= OLED_CHN_CHAR_WIDTH)
		{
			...
			{
				/*找到匹配的汉字*/
				if (strcmp(OLED_CF10x10[pIndex].Index, SingleChinese) == 0)
				{
					break;		//跳出循环，此时pIndex的值为指定汉字的索引
				}
			}
			...
			/*将汉字字模库OLED_CF16x16的指定数据以16*16的图像格式显示*/
			OLED_ShowImage(X + ((i + 1) / OLED_CHN_CHAR_WIDTH - 1) * 16, Y, 10, 10, OLED_CF10x10[pIndex].Data3);
		}
	}
}
...
169,199,226,253,245,42,145,103,109,135,59,25,1,96,173,162,158,53,175,118,208,156,11,247,6,181,27,202,217,239,119,176,174,16,127,
13,189,237,206,112,153,148,235,213,33,183,83,219,108,17,36,69,232,228,224,160,121,26,94,99,164,214,147,81,114,98,57,190,195,
111,100,64,38,31,140,49,249,185,170,216,251,115,172,215,67,24,254,20,19,82,73,161,192,87,76,35,71,225,209,182,152,58,250,123,
70,61,222,133,10,2,86,171,178,128,150,39,154,240,22,205,236,244,110,243,116,54,177,220,
};




void wave(uint8_t step)
{
	uint8_t PointX,PointY;
	uint8_t Xtime;
	uint8_t range = 63;
	float scale = (float)range / 255;
	uint8_t PointXlast,PointYlast;
	PointX=0;
	PointY=wavedata[0]*scale;
	...
	for(Xtime=0;Xtime<128;Xtime+=step)
	{
		...
		PointX=Xtime;
		PointY=wavedata[Xtime]+scale;
		
		OLED_DrawLine(PointXlast,63-PointYlast,PointX,63-PointY);
...
void Wave(uint8_t step)
{
	...
	for(Xtime=0;Xtime<127;Xtime+=step)
	{
		OLED_DrawLine(Xtime,63-WaveData[Xtime],Xtime+1,63-WaveData[Xtime+1]);
	}
