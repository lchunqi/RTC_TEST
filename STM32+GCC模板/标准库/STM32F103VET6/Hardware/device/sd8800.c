...
//rtc读addr
rtc_exit_code SD8800_read_addr(uint8_t reg_addr, uint8_t *read_buf, uint8_t length, i2c_device_bus bus)
{
	if(IIC_read_multiple_bytes(0x64,reg_addr,read_buf,length,bus)!=IIC_OK)
	{
		return RTC_NOACK;
	}
	return RTC_OK;
...
//rtc写addr
rtc_exit_code SD8800_write_addr(uint8_t reg_addr, uint8_t *write_buf, uint8_t length, i2c_device_bus bus)
{
	if(IIC_write_multiple_bytes(0x64,reg_addr,write_buf,length,bus)!=IIC_OK)
	{
		return RTC_NOACK;
	}
	return RTC_OK;
...
rtc_exit_code SD8800_ReadTimeDate(rtc_time *time, i2c_device_bus IIC_x)
{
	...
	if(SD8800_read_addr(0x00,buf,7,IIC_x)!=RTC_OK)
	{
		return RTC_FAIL;
	}
...
rtc_exit_code SD8800_WriteTimeDate(rtc_time *time, i2c_device_bus IIC_x)
{
	...
	if(SD8800_write_addr(0x00,buf,7,IIC_x)!=RTC_OK)
	{
		return RTC_FAIL;
	}
...
//读取芯片id码
uint8_t sd8800_idbuff(uint8_t *Data, i2c_device_bus IIC_x)
{
...
uint8_t sd8800_id_show(i2c_device_bus IIC_x)
{
		...
		if(sd8800_idbuff(ifbuff,IIC_x)!=TRUE)
			return FALSE;
...
uint8_t sd8800_ReadTemp(i2c_device_bus IIC_x)
{
	...
	uint8_t buf = 0x10;
	if(SD8800_write_addr(0x57,&buf,1,IIC_x)!=RTC_OK)
		return RTC_NOACK;
	
	Temp2 = RTC_ReadByte(0x17,IIC_x);
	
	sd8800_temp = ((Temp2 *2)-187.19)/3.218;
	bcd_to_dec(sd8800_temp);
...
{
	uint8_t	Vbatbit8,Vbatlow7;
	
	uint8_t buf = 0x10;
	if(SD8800_write_addr(0x59,&buf,1,IIC_x)!=RTC_OK)
		return RTC_NOACK;
	
	Vbatbit8 = RTC_ReadByte(0x5A,IIC_x);
	Vbatlow7 = RTC_ReadByte(0x5B,IIC_x);
	
	sd8800_vbat = ((Vbatbit8&0x80)>>7)*255+Vbatlow7;
	
	sd8800_vbat = sd8800_vbat/100;
	bcd_to_dec(sd8800_vbat);
...
//读取vbat电压和上位机使用
uint8_t sd8800_VbatQuan(uint8_t *data,i2c_device_bus IIC_x)
{
	uint8_t time = 5;
	...
	while(time--)
	{
		...
		SD8800_read_addr(0x5A,data,2,IIC_x);
			return TRUE;
...
{	
		...
		uint8_t buf=0x0C;
		SD8800_write_addr(0x0D,&buf,1,IIC_x);
...
uint8_t sd8800_and_std(void)
{
	...
	if(SD8800_ReadTimeDate(&time_buf,IIC_TEST)!=RTC_FAIL)
	{
...
uint8_t sd8800_time_show(void)
{
	...
	if(SD8800_ReadTimeDate(&time_bufstd,IIC_TEST)!=RTC_FAIL)
	{
		...
		sd8800_id_show(IIC_TEST);
 
		sd8800_ReadTemp(IIC_TEST);
		sd8800_ReadVbat(IIC_TEST);

		OLED_ShowNum(36,43,sd8800_temp,2,6);
		OLED_ShowChar(49,43,'C',6);
		OLED_DrawPoint(49,41);
			
		OLED_ShowString(68,43,"Vbat:",6);		
		OLED_ShowFloatNum(92,43,sd8800_vbat,1,2,6);
...
uint8_t sd8800_time_copy(void)
{
	...
	if(SD8800_WriteTimeDate(&copy_buf,IIC_TEST)!=RTC_FAIL)
	{
...
uint8_t RTC_ReadRAM_8800(uint8_t addr,uint8_t *data,uint8_t len,i2c_device_bus IIC_x)
{

	return SD8800_read_addr(addr,data,len,IIC_x);
	
}

uint8_t RTC_WriteRAM_8800(uint8_t addr,uint8_t *data,uint8_t len,i2c_device_bus IIC_x)
{

	 if(SD8800_write_addr(addr,data,len,IIC_x)!=RTC_OK)
		return RTC_NOACK;
...
{
	uint8_t buf=0x04;
	SD8800_write_addr(0x18,&buf,1,IIC_TEST);
