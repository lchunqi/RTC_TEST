...
uint8_t read_button_GPIO(uint8_t button_id)
{
	...
	{
		case btn1_id:
			return HAL_GPIO_ReadPin(GPIOA, GPIO_PIN_0);//1
		case btn2_id:
			return HAL_GPIO_ReadPin(GPIOC, GPIO_PIN_13);//2
		default:
			return 0;
	}
}
...
{
	PressEvent FLAG=66;
	FLAG=get_button_event(btn);
	UsbPrintf("FLAG=%d key1 PRESS_DOWN \r\n",FLAG);
}

void BTN1_LONG_PRESS_START_Handler(void* btn)
{
	UsbPrintf(" key1 LONG_PRESS_START\r\n");
}
void BTN1_DOUBLE_CLICK_Handler(void* btn)
{
	UsbPrintf(" key1 DOUBLE_CLICK\r\n");
}

void BTN2_PRESS_DOWN_Handler(void* btn)
{
	UsbPrintf(" key2 PRESS_DOWN\r\n");
}

void BTN2_DOUBLE_CLICK_Handler(void* btn)
{
	UsbPrintf(" key2 DOUBLE_CLICK\r\n");
}
