...
//   // 设置定时器，定时扫描串口列表
//   connect(timerSerial, &QTimer::timeout, this, &RtcMessage::updateComPortList);
//   timerSerial->start(1000);//定时1s更新一次

//    // 信号槽连接，当下拉框选择变化时执行切换串口操作
//    connect(ui->com_connect, QOverload<int>::of(&QComboBox::currentIndexChanged), [&](int index){
//    if(serialPort->isOpen()) {
//    serialPort->close(); // 如果串口已经打开，先关闭
//    }
//    // 获取所选串口名称
//    QString selectedPortName = ui->com_connect->currentText();
//    // 设置串口名字
//    serialPort->setPortName(selectedPortName);
//    // 打开串口
//    if(serialPort->open(QIODevice::ReadWrite)) {
//    qDebug("Switched to port: %s", selectedPortName.toStdString().c_str());
//    // 在这里可以进行串口通信等操作
//    } else {
//    qDebug("Failed to switch to port: %s", selectedPortName.toStdString().c_str());
//    }
...
//        connect(sd8563model, &QSqlTableModel::dataChanged, [=](const QModelIndex &topLeft, const QModelIndex &bottomRight)
//        {
//            if (topLeft.column() >= 3 && bottomRight.column() <= 10) // 判断是否在第4~11列输入
//            {
//                // 在第4~11列输入二进制数据，解析为16进制数据并设置到第12列
//                for (int row = topLeft.row(); row <= bottomRight.row(); row++)
//                {
//                    QString binaryString;
//                    for (int col = 3; col <= 10; col++) // 列范围应该是3到10
//                    {
//                        binaryString += sd8563model->data(sd8563model->index(row, col)).toString();
//                    }

//                    QByteArray bcdData;
//                    for (int i = 0; i < binaryString.length(); i += 8)
//                    {
//                        QString binary = binaryString.mid(i, 8);
//                        bool ok;
...
//                        }
//                    }
//                    QString finalString = bcdData.toHex() + "H"; // 转换为十六进制字符串并添加字符"H"
//                    // 设置第12列的数据为最终的十六进制字符串
//                    sd8563model->setData(sd8563model->index(row, 11), finalString);
...
//                if (topLeft.column() <= column && bottomRight.column() >= column)
//                {
//                    for (int outerRow = 1; outerRow <= 31; outerRow += 2)
//                    {
//                        QString hexInput = sd8563model->data(sd8563model->index(outerRow, 11)).toString();
...
//                                int decimalValue = hexInput.toInt(&ok, 16); // 将16进制字符串转换为整数
//                                QString binaryValue1 = QString::number(decimalValue, 2).rightJustified(8, '0'); // 将整数转换为8位二进制字符串
//                                sd8563model->setData(sd8563model->index(outerRow, col), binaryValue1.mid(col - 3, 1));
//                            }
//                        }
//                    }
//                }
...
//    connect(sd8825model, &QSqlTableModel::dataChanged, [=](const QModelIndex &topLeft, const QModelIndex &bottomRight)
//    {
//        for (int row = topLeft.row(); row <= bottomRight.row(); ++row)
//        {
//            int column = 11; // 检查第12列（index为11）的数据变化
//            if (topLeft.column() <= column && bottomRight.column() >= column)
//            {
//                for (int outerRow = 1; outerRow <= 500; outerRow += 2)
//                {
//                    QString hexInput = sd8825model->data(sd8825model->index(outerRow, 11)).toString();
...
//                            int decimalValue = hexInput.toInt(&ok, 16); // 将16进制字符串转换为整数
//                            QString binaryValue1 = QString::number(decimalValue, 2).rightJustified(8, '0'); // 将整数转换为8位二进制字符串
//                            sd8825model->setData(sd8825model->index(outerRow, col), binaryValue1.mid(col - 3, 1));
//                        }
//                    }
//                }
//            }
...
//    connect(sd8804model, &QSqlTableModel::dataChanged, [=](const QModelIndex &topLeft, const QModelIndex &bottomRight)
//    {
//        for (int row = topLeft.row(); row <= bottomRight.row(); ++row)
//        {
//            int column = 11; // 检查第12列（index为11）的数据变化
//            if (topLeft.column() <= column && bottomRight.column() >= column)
//            {
//                for (int outerRow = 1; outerRow <= 196; outerRow++)
//                {
//                    QString hexInput = sd8804model->data(sd8804model->index(outerRow, 11)).toString();
//                    if (outerRow <= 79 && outerRow % 2 != 0) {
//                        for (int col = 3; col < 11; col++)
//                        {
//                            if(hexInput != "0")
//                            {
...
//                                int decimalValue = hexInput.toInt(&ok, 16); // 将16进制字符串转换为整数
//                                QString binaryValue1 = QString::number(decimalValue, 2).rightJustified(8, '0'); // 将整数转换为8位二进制字符串
//                                sd8804model->setData(sd8804model->index(outerRow, col), binaryValue1.mid(col - 3, 1));
//                            }
//                        }
//                    }else if (outerRow >= 80 && outerRow <= 188) {
//                        for (int col = 3; col < 11; col++)
//                        {
//                            if(hexInput != "0")
//                            {
...
//                            }
//                        }
//                    }else if(outerRow >= 190 && outerRow % 2 == 0){
//                        for (int col = 3; col < 11; col++)
//                        {
//                            if(hexInput != "0")
//                            {
...
//    connect(sd8840model, &QSqlTableModel::dataChanged, [=](const QModelIndex &topLeft, const QModelIndex &bottomRight)
//    {
//        for (int row = topLeft.row(); row <= bottomRight.row(); ++row)
//        {
//            int column = 11; // 检查第12列（index为11）的数据变化
...
//                for (int outerRow = 1; outerRow <= 500; outerRow += 2)
//                {
//                    QString hexInput = sd8840model->data(sd8840model->index(outerRow, 11)).toString();
...
//                            int decimalValue = hexInput.toInt(&ok, 16); // 将16进制字符串转换为整数
//                            QString binaryValue1 = QString::number(decimalValue, 2).rightJustified(8, '0'); // 将整数转换为8位二进制字符串
//                            sd8840model->setData(sd8840model->index(outerRow, col), binaryValue1.mid(col - 3, 1));
//                        }
//                    }
//                }
//            }
...
//    connect(sd8902model, &QSqlTableModel::dataChanged, [=](const QModelIndex &topLeft, const QModelIndex &bottomRight)
//    {
//        for (int row = topLeft.row(); row <= bottomRight.row(); ++row)
//        {
//            int column = 11; // 检查第12列（index为11）的数据变化
...
//                for (int outerRow = 1; outerRow <= 500; outerRow += 2)
//                {
//                    QString hexInput = sd8902model->data(sd8902model->index(outerRow, 11)).toString();
...
//                            int decimalValue = hexInput.toInt(&ok, 16); // 将16进制字符串转换为整数
//                            QString binaryValue1 = QString::number(decimalValue, 2).rightJustified(8, '0'); // 将整数转换为8位二进制字符串
//                            sd8902model->setData(sd8902model->index(outerRow, col), binaryValue1.mid(col - 3, 1));
//                        }
//                    }
//                }
//            }
...
//    connect(sd8931model, &QSqlTableModel::dataChanged, [=](const QModelIndex &topLeft, const QModelIndex &bottomRight)
//    {
//        for (int row = topLeft.row(); row <= bottomRight.row(); ++row)
//        {
//            int column = 11; // 检查第12列（index为11）的数据变化
...
//                for (int outerRow = 1; outerRow <= 500; outerRow += 2)
//                {
//                    QString hexInput = sd8931model->data(sd8931model->index(outerRow, 11)).toString();
...
//                            int decimalValue = hexInput.toInt(&ok, 16); // 将16进制字符串转换为整数
//                            QString binaryValue1 = QString::number(decimalValue, 2).rightJustified(8, '0'); // 将整数转换为8位二进制字符串
//                            sd8931model->setData(sd8931model->index(outerRow, col), binaryValue1.mid(col - 3, 1));
//                        }
//                    }
//                }
//            }
...
//    connect(sd8810model, &QSqlTableModel::dataChanged, [=](const QModelIndex &topLeft, const QModelIndex &bottomRight)
//    {
//        for (int row = topLeft.row(); row <= bottomRight.row(); ++row)
//        {
//            int column = 11; // 检查第12列（index为11）的数据变化
...
//                for (int outerRow = 1; outerRow <= 500; outerRow += 2)
//                {
//                    QString hexInput = sd8810model->data(sd8810model->index(outerRow, 11)).toString();
...
//                            int decimalValue = hexInput.toInt(&ok, 16); // 将16进制字符串转换为整数
//                            QString binaryValue1 = QString::number(decimalValue, 2).rightJustified(8, '0'); // 将整数转换为8位二进制字符串
//                            sd8810model->setData(sd8810model->index(outerRow, col), binaryValue1.mid(col - 3, 1));
//                        }
//                    }
//                }
//            }
...
//    connect(sd8939model, &QSqlTableModel::dataChanged, [=](const QModelIndex &topLeft, const QModelIndex &bottomRight)
//    {
//        for (int row = topLeft.row(); row <= bottomRight.row(); ++row)
//        {
//            int column = 11; // 检查第12列（index为11）的数据变化
...
//                for (int outerRow = 1; outerRow <= 500; outerRow += 2)
//                {
//                    QString hexInput = sd8939model->data(sd8939model->index(outerRow, 11)).toString();
...
//                            int decimalValue = hexInput.toInt(&ok, 16); // 将16进制字符串转换为整数
//                            QString binaryValue1 = QString::number(decimalValue, 2).rightJustified(8, '0'); // 将整数转换为8位二进制字符串
//                            sd8939model->setData(sd8939model->index(outerRow, col), binaryValue1.mid(col - 3, 1));
//                        }
//                    }
//                }
//            }
...
//    connect(sd8800model, &QSqlTableModel::dataChanged, [=](const QModelIndex &topLeft, const QModelIndex &bottomRight)
//    {
//        for (int row = topLeft.row(); row <= bottomRight.row(); ++row)
//        {
//            int column = 11; // 检查第12列（index为11）的数据变化
//            if (topLeft.column() <= column && bottomRight.column() >= column)
//            {
//                for (int outerRow = 1; outerRow <= 500; outerRow++)
//                {
//                    QString hexInput = sd8800model->data(sd8800model->index(outerRow, 11)).toString();
//                    if (outerRow <= 53 && outerRow % 2 != 0) {
//                        for (int col = 3; col < 11; col++)
//                        {
//                            if(hexInput != "0")
//                            {
...
//                                int decimalValue = hexInput.toInt(&ok, 16); // 将16进制字符串转换为整数
//                                QString binaryValue1 = QString::number(decimalValue, 2).rightJustified(8, '0'); // 将整数转换为8位二进制字符串
//                                sd8800model->setData(sd8800model->index(outerRow, col), binaryValue1.mid(col - 3, 1));
//                            }
//                        }
//                    }else if (outerRow >= 54 && outerRow <= 162) {
//                        for (int col = 3; col < 11; col++)
//                        {
//                            if(hexInput != "0")
//                            {
...
//                            }
//                        }
//                    }else if(outerRow >= 163 && outerRow % 2 == 0){
//                        for (int col = 3; col < 11; col++)
//                        {
//                            if(hexInput != "0")
//                            {
...
//    connect(sd8200model, &QSqlTableModel::dataChanged, [=](const QModelIndex &topLeft, const QModelIndex &bottomRight)
//    {
//        for (int row = topLeft.row(); row <= bottomRight.row(); ++row)
//        {
//            int column = 11; // 检查第12列（index为11）的数据变化
...
//                for (int outerRow = 1; outerRow <= 500; outerRow += 2)
//                {
//                    QString hexInput = sd8200model->data(sd8200model->index(outerRow, 11)).toString();
...
//                            int decimalValue = hexInput.toInt(&ok, 16); // 将16进制字符串转换为整数
//                            QString binaryValue1 = QString::number(decimalValue, 2).rightJustified(8, '0'); // 将整数转换为8位二进制字符串
//                            sd8200model->setData(sd8200model->index(outerRow, col), binaryValue1.mid(col - 3, 1));
//                        }
//                    }
//                }
//            }
...
//        connect(sd2010model, &QSqlTableModel::dataChanged, [=](const QModelIndex &topLeft, const QModelIndex &bottomRight)
//        {
//                for (int row = topLeft.row(); row <= bottomRight.row(); ++row)
//                {
//                    for (int col = topLeft.column(); col <= bottomRight.column(); ++col)
//                    {
//                        if (topLeft.column() <= 10 && bottomRight.column() >= 3)
//                        {
//                         int column = 11; // 检查第12列（index为11）的数据变化
//                         if (topLeft.column() <= column && bottomRight.column() >= column)
//                         {
...
//                                  for (int col = 3; col < 11; col++)
//                                  {
//                                      QString hexInput = sd2010model->data(sd2010model->index(row, 11)).toString();
//                                      bool ok;
//                                      int decimalValue = hexInput.toInt(&ok, 16); // 将16进制字符串转换为整数
//                                      QString binaryValue1 = QString::number(decimalValue, 2).rightJustified(8, '0'); // 将整数转换为8位二进制字符串
//                                      sd2010model->setData(sd2010model->index(row, col), binaryValue1.mid(col-3, 1));
//                                  }
//                             }
//                         }
//                      }
...
//                for (int outerRow = 1; outerRow <= 500; outerRow += 2)
//                {
//                    QString hexInput = sd2010model->data(sd2010model->index(outerRow, 11)).toString();
...
//                            int decimalValue = hexInput.toInt(&ok, 16); // 将16进制字符串转换为整数
//                            QString binaryValue1 = QString::number(decimalValue, 2).rightJustified(8, '0'); // 将整数转换为8位二进制字符串
//                            sd2010model->setData(sd2010model->index(outerRow, col), binaryValue1.mid(col - 3, 1));
//                        }
//                    }
//                }
//            }
...
//    connect(sd2058model, &QSqlTableModel::dataChanged, [=](const QModelIndex &topLeft, const QModelIndex &bottomRight)
//    {
//        for (int row = topLeft.row(); row <= bottomRight.row(); ++row)
//        {
//            int column = 11; // 检查第12列（index为11）的数据变化
...
//                for (int outerRow = 1; outerRow <= 500; outerRow += 2)
//                {
//                    QString hexInput = sd2058model->data(sd2058model->index(outerRow, 11)).toString();
...
//                            int decimalValue = hexInput.toInt(&ok, 16); // 将16进制字符串转换为整数
//                            QString binaryValue1 = QString::number(decimalValue, 2).rightJustified(8, '0'); // 将整数转换为8位二进制字符串
//                            sd2058model->setData(sd2058model->index(outerRow, col), binaryValue1.mid(col - 3, 1));
//                        }
//                    }
//                }
//            }
...
//    connect(sd2059model, &QSqlTableModel::dataChanged, [=](const QModelIndex &topLeft, const QModelIndex &bottomRight)
//    {
//        for (int row = topLeft.row(); row <= bottomRight.row(); ++row)
//        {
//            int column = 11; // 检查第12列（index为11）的数据变化
...
//                for (int outerRow = 1; outerRow <= 500; outerRow += 2)
//                {
//                    QString hexInput = sd2059model->data(sd2059model->index(outerRow, 11)).toString();
...
//                            int decimalValue = hexInput.toInt(&ok, 16); // 将16进制字符串转换为整数
//                            QString binaryValue1 = QString::number(decimalValue, 2).rightJustified(8, '0'); // 将整数转换为8位二进制字符串
//                            sd2059model->setData(sd2059model->index(outerRow, col), binaryValue1.mid(col - 3, 1));
//                        }
//                    }
//                }
//            }
...
//    connect(sd2068model, &QSqlTableModel::dataChanged, [=](const QModelIndex &topLeft, const QModelIndex &bottomRight)
//    {
//        for (int row = topLeft.row(); row <= bottomRight.row(); ++row)
//        {
//            int column = 11; // 检查第12列（index为11）的数据变化
...
//                for (int outerRow = 1; outerRow <= 500; outerRow += 2)
//                {
//                    QString hexInput = sd2068model->data(sd2068model->index(outerRow, 11)).toString();
...
//                            int decimalValue = hexInput.toInt(&ok, 16); // 将16进制字符串转换为整数
//                            QString binaryValue1 = QString::number(decimalValue, 2).rightJustified(8, '0'); // 将整数转换为8位二进制字符串
//                            sd2068model->setData(sd2068model->index(outerRow, col), binaryValue1.mid(col - 3, 1));
//                        }
//                    }
//                }
//            }
...
//    connect(sd2069model, &QSqlTableModel::dataChanged, [=](const QModelIndex &topLeft, const QModelIndex &bottomRight)
//    {
//        for (int row = topLeft.row(); row <= bottomRight.row(); ++row)
//        {
//            int column = 11; // 检查第12列（index为11）的数据变化
...
//                for (int outerRow = 1; outerRow <= 500; outerRow += 2)
//                {
//                    QString hexInput = sd2069model->data(sd2069model->index(outerRow, 11)).toString();
...
//                            int decimalValue = hexInput.toInt(&ok, 16); // 将16进制字符串转换为整数
//                            QString binaryValue1 = QString::number(decimalValue, 2).rightJustified(8, '0'); // 将整数转换为8位二进制字符串
//                            sd2069model->setData(sd2069model->index(outerRow, col), binaryValue1.mid(col - 3, 1));
//                        }
//                    }
//                }
//            }
...
//    connect(sd2077model, &QSqlTableModel::dataChanged, [=](const QModelIndex &topLeft, const QModelIndex &bottomRight)
//    {
//        for (int row = topLeft.row(); row <= bottomRight.row(); ++row)
//        {
//            int column = 11; // 检查第12列（index为11）的数据变化
...
//                for (int outerRow = 1; outerRow <= 500; outerRow += 2)
//                {
//                    QString hexInput = sd2077model->data(sd2077model->index(outerRow, 11)).toString();
...
//                            int decimalValue = hexInput.toInt(&ok, 16); // 将16进制字符串转换为整数
//                            QString binaryValue1 = QString::number(decimalValue, 2).rightJustified(8, '0'); // 将整数转换为8位二进制字符串
//                            sd2077model->setData(sd2077model->index(outerRow, col), binaryValue1.mid(col - 3, 1));
//                        }
//                    }
//                }
//            }
...
//    connect(sd2078model, &QSqlTableModel::dataChanged, [=](const QModelIndex &topLeft, const QModelIndex &bottomRight)
//    {
//        for (int row = topLeft.row(); row <= bottomRight.row(); ++row)
//        {
//            int column = 11; // 检查第12列（index为11）的数据变化
...
//                for (int outerRow = 1; outerRow <= 500; outerRow += 2)
//                {
//                    QString hexInput = sd2078model->data(sd2078model->index(outerRow, 11)).toString();
...
//                            int decimalValue = hexInput.toInt(&ok, 16); // 将16进制字符串转换为整数
//                            QString binaryValue1 = QString::number(decimalValue, 2).rightJustified(8, '0'); // 将整数转换为8位二进制字符串
//                            sd2078model->setData(sd2078model->index(outerRow, col), binaryValue1.mid(col - 3, 1));
//                        }
//                    }
//                }
//            }
...
//        if(text=="SD3068")
//        {
//            sd3068model->revertAll();
//            // 重新加载数据，以确保显示原始数据
//            sd3068model->select();
//        }else if(text=="SD8563"){
//            sd8563model->revertAll();
//            sd8563model->select();
//        }else if(text=="SD8825"){
//            sd8825model->revertAll();
//            sd8825model->select();
//        }else if(text=="SD8840"){
//            sd8840model->revertAll();
//            sd8840model->select();
//        }else if(text=="SD8902"){
//            sd8902model->revertAll();
//            sd8902model->select();
//        }else if(text=="SD8931"){
//            sd8931model->revertAll();
//            sd8931model->select();
//        }else if(text=="SD8810"){
//            sd8810model->revertAll();
//            sd8810model->select();
//        }else if(text=="SD8939"){
//            sd8939model->revertAll();
//            sd8939model->select();
//        }else if(text=="SD8800"){
//            sd8800model->revertAll();
//            sd8800model->select();
//        }else if(text=="SD8200"){
//            sd8200model->revertAll();
//            sd8200model->select();
//        }
...
//        {
//            // 提交所有更改到数据库
//            sd3068model->submitAll();
//        }else if(text=="SD8563"){
//            sd8563model->submitAll();
//        }else if(text=="SD8825"){
//            sd8825model->submitAll();
//        }else if(text=="SD8840"){
//            sd8840model->submitAll();
//        }else if(text=="SD8902"){
//            sd8902model->submitAll();
//        }else if(text=="SD8804"){
//            sd8804model->submitAll();
//        }else if(text=="SD8931"){
//            sd8931model->submitAll();
//        }else if(text=="SD8810"){
//            sd8810model->submitAll();
//        }else if(text=="SD8939"){
//            sd8939model->submitAll();
//        }else if(text=="SD8800"){
//            sd8800model->submitAll();
//        }else if(text=="SD8200"){
//            sd8200model->submitAll();
//        }
            ...
            QTimer::singleShot(100, [&](){
                ...
                if (!readtime.isEmpty()) {
                       ...
                       if (readtime.size() == 8) {
                           ...
                           // 转换为十进制时间
                           QString decimalTime = QString("20%1年%2月%3日 星期:%7  %4:%5:%6 ")
                                                     .arg(year)
                                                     .arg(month)
                                                     .arg(day)
        ...
        //        }                
                    if (!ROOdata.isEmpty())
                    {
                       if(ROOdata.size()==33)
                       {
                               ...
                               for (int i = 0; i < ROOdata.size() && i < 32; i++) {
...
void RtcMessage::on_W00H_clicked()
{
    ...
    if(serialPort->isOpen())
    {
        if(text=="SD3068"||text=="SD3010"||text=="SD3025"||text=="SD3031"||text=="SD3077"||text=="SD3078"||text=="SD3178"||text=="SD3900"||text=="SD2503"||text=="SD2505"||text=="SD2506"||text=="SD2507"||text=="SD2508"||text=="SD2520"||text=="SD2522"||text=="SD2523")
        {
            sd3068Modelselt();
            QString binaryData;
                    for (int row = 1; row <= 13; row += 2)//row = 1(实际行号)--->数据表中行为2，直接看数据表，找到需要的行号，行-1即为实际需要的行号，因为行从0开始索引
                    {
                        for (int col = 3; col < 11; col++)
                        {
