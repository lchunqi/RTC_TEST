...
void UsartInit(void)
{
	...
	rtc_group_power(g_rtcChanenel, POWER_I2C);
	ControlInit();
...
void AT_RTC4096_Handle(char* command)
{
	...
	if (inNumber > 0) {
		result = sd8825_out_freq();
	} else {
		result = sd8825_close_freq();
	}
	if (result == I2C_OK) {
		UsbPrintf("\nOK\n");
	} else {
...
void AT_RTCADJCAP_Handle(char* command)
{
	...
	// rtc_freq_test(FREQ_TEST_STARTING);//自动调节
	...
	int list = 0;
	uint8_t buffer;
	CharToInt(command, strSearch(command, "=") + 1, &num);
	CharToInt(command, strSearch(command, ",") + 1, &list);
	if (num == 1) {
		buffer = (uint8_t)list;
		osc_buf[1] = (mode << 3) | buffer >> 5;
		osc_buf[0] = (buffer << 3) | (buffer & 0x07);
	} else if (num == 2) {
		rtc_freq_test(FREQ_TEST_STARTING);//自动调节
	} else {
		cap_list_to_uint(list, osc_buf);
	}
	if (rtc_set_testcap(osc_buf) != I2C_OK) { //写ram22、ram23 
		UsbPrintf("adjust error\n");
		return;
...
void AT_FREQTEST_Handle(char* command)
{
	...
	for (i = 0; i < 15; i++) {
		HAL_Delay(100);
		if (strlen((char*)g_DeviceRxBuff) > 0) {
			UsbPrintf("%s\n", (char*)g_DeviceRxBuff);
			if (strSearch((char*)g_DeviceRxBuff, "Hz") > 0 || strSearch((char*)g_DeviceRxBuff, "PPM") > 0) {
				break;
			}
		}
	}
...
void AT_READGROUP_Handle(char* command)
{
	...
	if (CharToInt(command, 13, &channel) == 1) {
		...
		if (g_rtcChanenel != channel) {
			HAL_Delay(500);
			g_rtcChanenel = channel;
		}
		state = rtc_read_multi_bytes(0x64, reg, result, 8);
		UsbPrintf("read group[%d] state[%d] reg[0x%x]:", channel, state, reg);
		for (i = 0; i < 8; i++) {
...
void AT_RESULT_Handle(char* command)//取料机查询信息
{
	...
	if (strSearch((char*)g_DeviceRxBuff, "AT^RESULT?") == 0 || strSearch((char*)g_UsbRxBuff, "AT^RESULT?") == 0) {
...
{	
	...
	for (j = 0; j < 50; j++) {
		...
		rtc_group_power(j + 1, POWER_I2C);
		rtc_set_channel(j + 1, RTC_SD8825);
		g_rtcChanenel = j + 1;
		HAL_Delay(10);
		state = rtc_read_temp(&temp);
		if (state == I2C_OK && temp > 10) {
			UsbPrintf("1\t");
		} else {
			UsbPrintf("0\t");
		}
...
void AT_1006TMODE_Handle(char* command)
{
	...
	if (mode > 0) {
		...
		HAL_Delay(5);
		rtc_set_channel(g_rtcChanenel, RTC_SD8825);
...
void AT_SD8825INT_Handle(char* command)
{
	...
	if (startNumber > 0) {
		...
		for (i = startNumber; i <= endNumber; i++) {
			...
			rtc_group_power(i, POWER_TEST);
			rtc_set_channel(i, RTC_SD8825);
			g_rtcChanenel = i;
			HAL_Delay(1);
			rtc_read_temp(&temp);
			sysTemp = temp_read_value(1);
			if ((temp == 0 || temp == -0.5) && (sysTemp > 5 || sysTemp < -5)) {//用ACK判断是不准确的
			...
			for (j = 0; j < 40; j++) {
				...
				if (num > 0) {
					...
					ppm = (freq / CENTER_FREQENCY - 1.0) * 1000000;
					UsbPrintf("chip%d temp:%.1f, system temp = %.1fC, ppm:%.2f\n", g_rtcChanenel, temp, sysTemp, ppm);
					memset(g_DeviceRxBuff, 0, UART_RECEIVE_BUFF_NUMBER);
					device_Rx_Cnt = 0;
					break;
				} 
...
void AT_RTCLOAD_Handle(char* command)
{
	...
	CharToInt(command, strSearch(command, "=") + 1, &number);
	if (number > 0) {
		sd8825_write_otp(number);
	} else {
		for (i = 1; i <= MAX_CHIP_NUMBER; i++) {
			result[i - 1] = sd8825_write_otp(i);
			HAL_Delay(1000);
		}
