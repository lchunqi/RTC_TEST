...
float initCapExample[TEST_NUMBER] = {220, 380, 480, 380, 220};
uint8_t calcul_read_data(uint8_t rtcNum)
{
	...
	g_result[TEST_RTC_TEMP_Z85] -= 4;						//2024.08.09 chenxuezhong 依据实测数据修正，使得中心点上移1.6ppm 
...
uint8_t init_para_5dot(void)
{
	...
	if ((minError >= 0 && maxError < 0 ) || (minError <= 0 && maxError > 0) || fabs(maxError) < 0.1) {//存在中间解
		...
		UsbPrintf("min:%.2f, max:%.2f\n", minError, maxError);
		if ((minError > 0 && maxError < 0 ) || (minError < 0 && maxError > 0) || fabs(maxError) < 0.1) {
...
//-------------------------------------------------------------------------------------------------------------------
	...
	log_printf("parameter init success!\n");
...
uint8_t calculate_wave1006_temp_cap(uint8_t rtcNum, float* cap_value, int* digtal, uint16_t* offset)
{
	...
	*offset = g_testData.tempConfig;
    ...
    for (i = 0; i < 420; i++) {
		...
		// UsbPrintf("%d: %d, %.2f\n", adcStart + i, digtal[i], cap_value[i]);
