	...
	if (num >= 0) {
		...
		switch (inNumber) {
			...
			case OPERATE_CAPADJUST_COVER:
				ControlPrintf("okNumber:0 chipName:0 message:清空再单点测试...\n");
			...
			case OPERATE_MACHINE_WRITE:
				ControlPrintf("okNumber:0 chipName:0 message:无此模式\n");
				manager_set_mode(MACHINE_WRITE);
			break;
			case OPERATE_MACHINE_COVER:
				ControlPrintf("okNumber:0 chipName:0 message:先清空再机台测试\n");
