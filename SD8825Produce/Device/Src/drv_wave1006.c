...
wave1006_otp_arg_t g_sd8825_config = {
	...
	PKG_RX8025_RXD8900,		//package_type_config
	...
	0,				//event_support_config 			//support event extend
	0,				//pps_support_config					//support 1 pps extend
	...
	0,				//charge_ext_config					//1:支持扩展充电  0:不支持扩展充电
	0,				//bat_val_ext_config
	0,				//rst_out_support_config 
	1,				//temp_alarm_ext_config
	1,				//temp_val_ext_config
	5,  			//rule_temp_adj_config
  	0,				//rule_bat_adj_config
	2,				//osc_cap_sel_config1        		//封装电容选择值 8825选2 2024.01.25cxz
	1,    		 	//adjust_temp_1_3_ppm_config		//3ppm
	1,				//en_bat_pulldown_config     		//有BAT脚的配置为0,没有BAT脚的配置为1通过内部200k下拉到地
	...
	0, 				//en_charge_config						//1:支持充电  0:不支持充电
...
wave1006_exit_code_e wave1006_enter_test_mode(wave1006_test_mode_e mode)     //烧写1006时使用
{
	...
	/*2.set test mode*/
	buf[0] = mode;
	if(drv_wave1006_write_reg(DEV_SD8804, 0x07, 1, &buf[0]) != WAVE1006_EXIT_OK)//8825C版本
	{
		return WAVE1006_EXIT_ARG_ERR;
	}
	/*3. set time reg*/
...
wave1006_exit_code_e wave1006_write_otp(wave1006_otp_arg_t const *config_arg,  unsigned char const *id, wave1006_temp_arg_t const *temp_arg, uint8_t rtcNumber)
{
	...
	rtc_set_channel(rtcNumber, RTC_SD8825);
	HAL_Delay(100);
	buf1 = 0x40;
	if(drv_wave1006_write_reg(DEV_SD3068, WAVE1006_REG_CTR3, 1, &buf1) != WAVE1006_EXIT_OK)   //如果是有应该先关闭32K输出(默认为关闭SD3068的32K)
	{
		return WAVE1006_EXIT_FAIL;
	}
	if(drv_wave1006_read_reg(DEV_SD3068, WAVE1006_REG_USER_RAM_START + 20, 1, &buf1) != WAVE1006_EXIT_OK)   //选择为SD3031封装(必须在进测试模式之前选择，否则芯片pad改变)
	{
		return WAVE1006_EXIT_FAIL;
	}
	buf1 &= ~0x0f;
	buf1 |= 0x00;//根据pad配置表，编号0为8825
	if(drv_wave1006_write_reg(DEV_SD3068, WAVE1006_REG_USER_RAM_START + 20, 1, &buf1) != WAVE1006_EXIT_OK)
	{
		return WAVE1006_EXIT_FAIL;
	}
		/*clear RAM2~6*/
	if(drv_wave1006_write_reg(DEV_SD3068, WAVE1006_REG_USER_RAM_START + 1, 5, &buf[0]) != WAVE1006_EXIT_OK)
	{
		return WAVE1006_EXIT_FAIL;
	}
    ...
    /*switch i2c prot, io6 =5V*/
 	rtc_set_channel(rtcNumber, RTC_SD8825);//切换pad引脚
	HAL_Delay(10);
    /*vdd=1*/
  buf[0] = 0x80;
	if (drv_wave1006_write_reg(DEV_SD3068, WAVE1006_REG_USER_RAM_START + 1, 1, &buf[0])!= WAVE1006_EXIT_OK) {
...
//	write otp is complete, switch i2c prot, io6 =5V
	...
	if (check_sum_rd == check_sum_wr) {
		...
		otp_temp |= 0x05;                                    //写入封装形式并写入校验成功标志
		wave1006_write_otp_single(16, otp_temp);
		otp_temp = ((otp_buf[1] << 4) | 0);           //写入产品型号和晶振型号
		...
		wave1006_write_otp_single(0, 0x3ffe);              //写入保护位
	} else {
		UsbPrintf("write check is err %lx: cal=%lx\r\n",check_sum_rd, check_sum_wr);
		otp_temp = ((otp_buf[1] >> 6) & 0xf0);  
		otp_temp |= 0x0a;                                       //写入封装形式并写入校验成功标志
		wave1006_write_otp_single(16, otp_temp);
		otp_temp = ((otp_buf[1] << 4) | 0);           //写入产品型号和晶振型号
		wave1006_write_otp_single(17, otp_temp);
...
wave1006_exit_code_e wave1006_write_otp_one(char *str)
{
	...
	if(drv_wave1006_read_reg(DEV_SD3068, WAVE1006_REG_USER_RAM_START + 20, 1, &buf1) != WAVE1006_EXIT_OK)   //选择为SD8825封装(必须在进测试模式之前选择，否则芯片pad改变)
	{
		return WAVE1006_EXIT_FAIL;
	}
	buf1 &= ~0x0f;
...
wave1006_exit_code_e wave1006_read_otp(void)
{
	...
	if(drv_wave1006_read_reg(DEV_SD3068, WAVE1006_REG_USER_RAM_START + 20, 1, &buf1) != WAVE1006_EXIT_OK)   //选择为SD3302封装(必须在进测试模式之前选择，否则芯片pad改变)
	{
		return WAVE1006_EXIT_FAIL;
	}
	buf1 &= ~0x0f;
...
uint8_t sd8825_calcul_cap(uint8_t channel)
{
	int i;
	for (i = 0; i < 420; i++) { 
		g_cap_val[i] = 0; 
...
uint8_t make_sd8825_id(uint8_t channel)
{
	...
	CharToInt(bsn, 4, &number);//只取最后4位做机台号码
	if (number < 1) {
	...
	g_dev_id[0] = ((date.year / 10) % 10) * 16 + (date.year % 10);	//年
	g_dev_id[1] = ((date.mon / 10) % 10) * 16 + (date.mon % 10);	//月
	g_dev_id[2] = ((date.day / 10) % 10) * 16 + (date.day % 10);	//日
	g_dev_id[3] = ((number / 1000) % 10) * 16 + ((number / 100) % 10);		//板件编号
	g_dev_id[4] = (g_csn / 1000) % 10 * 16 + (g_csn / 100) % 10;	//0x24;//
	g_dev_id[5] = (g_csn / 10) % 10 * 16 + g_csn % 10;		//0x18;//		
	g_dev_id[6] = ((number / 10) % 10) * 16 + (number  % 10);		//板件编号
	g_dev_id[7] = ((channel / 10) % 10) * 16 + (channel % 10);		//序号
...
uint8_t sd8825_write_otp(uint8_t channel)
{
	int i;
	uint8_t buffer[2] = {0};
	int num_adj = 0;

	if (make_sd8825_id(channel) == 0) { 
		return 0; 
	}
	if (sd8825_calcul_cap(channel) == 0) { 
		return 0;
	}
	g_sd8825_config.temp_offset_config = g_offset;			 //温度偏移值，不同芯片不一样
	//烧写带温补的OTP    配置为SD8825   烧写保护位		
	for (i = 0; i < 420; i++) {
	...
	UsbPrintf("*************** WRITE SD8825 ************\r\n");
	if (wave1006_write_otp(&g_sd8825_config, g_dev_id, g_temp_arg, channel) != WAVE1006_EXIT_OK) {
		return 0;
	}
	return 1;
}
