...
void spi_receive_one_byte(uint8_t *buf)
{
	uint8_t i;
...
*******************************************************************/
void bcd_to_decimal(uint8_t *bcd,uint8_t times)
{
	uint8_t i;
	for(i=0;i<times;i++)
	{
...
*******************************************************************/
void decimal_to_bcd(uint8_t *dec,uint8_t times)
{
	uint8_t i;
	for(i=0;i<times;i++)
	{
...
uint8_t sd8902_write_buff[]={0x80,0x82,0x84,0x86,0x88,0x8A,0x8C,0x8E,0x90,0xB2,0xB4,0xBA,0xBC,0xC0,0xC2,0xC4,0xC6,0xC8,0xD0,0xD2,0xD4,0xD6,0xD8,0xDA,0xDC,0xDE,0xE0,0xE2,0xE4,0xE6,0xE8,0xEA,0xEC,0xEE,0xF0,0xF2,0xF4,0xF6,0xF8,0xFA,0xFC};
...
//    for(i = 0; i < len; i++)
//    {
//        buf[i] = sd8902_read_data(sd8902_read_buff[reg_addr + i]);
//    }
...
uint8_t sd8902_write_multi_bytes(uint8_t reg_addr, uint8_t *buf, uint8_t len)
{
	 ...
	 for(uint8_t i = 0; i < len; i++)
    {
        ...
        {
            sd8902_write_data(reg_addr,buf[sd8902_write_buff[index + i]]);
        }
        else
        {
            return FALSE;
