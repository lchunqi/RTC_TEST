...
//rtc读addr
rtc_exit_code SD8804_read_addr(uint8_t reg_addr, uint8_t *read_buf, uint8_t length, i2c_device_bus bus)
{
...
//rtc写addr
rtc_exit_code SD8804_write_addr(uint8_t reg_addr, uint8_t *write_buf, uint8_t length, i2c_device_bus bus)
{
...
rtc_exit_code SD8804_ReadTimeDate(rtc_time *time, i2c_device_bus IIC_x)
{
	...
	if(SD8804_read_addr(0x00,buf,7,IIC_x)!=RTC_OK)
	{
		return RTC_FAIL;
	}
...
rtc_exit_code SD8804_WriteTimeDate(rtc_time *time, i2c_device_bus IIC_x)
{
	...
	if(SD8804_write_addr(0x00,buf,7,IIC_x)!=RTC_OK)
	{
		return RTC_FAIL;
	}
...
//读取芯片id码
uint8_t sd8804_idbuff(uint8_t *Data, i2c_device_bus IIC_x)
{
...
uint8_t sd8804_id_show(i2c_device_bus IIC_x)
{
		...
		if(sd8804_idbuff(ifbuff,IIC_x)!=TRUE)
			return FALSE;
...
uint8_t sd8804_ReadTemp(i2c_device_bus IIC_x)
{
	...
	uint8_t buf = 0x10;
	if(SD8804_write_addr(0x57,&buf,1,IIC_x)!=RTC_OK)
		return RTC_NOACK;
	...
	if(Temp1 & 0x80)
	{
		Temp1 = (~Temp1+1)&0xFF;
		sd8804_temp = Temp1;
		bcd_to_dec(sd8804_temp);
		sd8804_temp = 256-sd8804_temp;
...
{	
		...
		uint8_t buf=0x0C;
		SD8804_write_addr(0x0D,&buf,1,IIC_x);
...
uint8_t sd8804_and_std(void)
{
	...
	if(SD8804_ReadTimeDate(&time_buf,IIC_TEST)!=RTC_FAIL)
	{
...
uint8_t sd8804_time_show(void)
{
	...
	if(SD8804_ReadTimeDate(&time_bufstd,IIC_TEST)!=RTC_FAIL)
	{
		...
		sd8804_id_show(IIC_TEST);
		sd8804_ReadTemp(IIC_TEST);

		OLED_ShowNum(36,43,sd8804_temp,2,6);
...
uint8_t sd8804_time_copy(void)
{
	...
	if(SD8804_WriteTimeDate(&copy_buf,IIC_TEST)!=RTC_FAIL)
	{
...
uint8_t RTC_ReadRAM_8804(uint8_t addr,uint8_t *data,uint8_t len,i2c_device_bus IIC_x)
{

	return SD8804_read_addr(addr,data,len,IIC_x);
	
}

uint8_t RTC_WriteRAM_8804(uint8_t addr,uint8_t *data,uint8_t len,i2c_device_bus IIC_x)
{

	 if(SD8804_write_addr(addr,data,len,IIC_x)!=RTC_OK)
		return RTC_NOACK;
