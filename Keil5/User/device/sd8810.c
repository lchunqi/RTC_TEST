...
返回值：TRUE-成功
**/
uint8_t sd8810_idbuff(uint16_t *Data, i2c_device_bus IIC_x)
{
...
{	
		uint16_t buf=0xC0;
		if(SD8810_write_addr(0x1D,&buf,1,IIC_x)!=RTC_OK)
			return RTC_FAIL;
	
		buf=0x00;
		if(SD8810_write_addr(0x32,&buf,1,IIC_x)!=RTC_OK)
			return RTC_FAIL;
...
返回值：sd8810_read_addr(addr,data,len,IIC_x)-读取到的数据
**/
