...
返回值：RTC_OK-成功
**/
rtc_exit_code SD8931_read_addr(uint8_t reg_addr, uint16_t *read_buf, uint8_t length, i2c_device_bus bus)
{
...
返回值：RTC_OK-成功
**/
rtc_exit_code SD8931_write_addr(uint8_t reg_addr, uint16_t *write_buf, uint8_t length, i2c_device_bus bus)
{
...
rtc_exit_code SD8931_ReadTimeDate(rtc_time *time, i2c_device_bus IIC_x)
{
	...
	if(SD8931_read_addr(0x00,buf,7,IIC_x)!=RTC_OK)
	{
		return RTC_FAIL;
	}
...
rtc_exit_code SD8931_WriteTimeDate(rtc_time *time, i2c_device_bus IIC_x)
{
	...
	if(SD8931_write_addr(0x00,buf,7,IIC_x)!=RTC_OK)
	{
		return RTC_FAIL;
	}
...
{	
		uint16_t buf=0x18;
		if(SD8931_write_addr(0x0E,&buf,1,IIC_x)!=RTC_OK)
			return RTC_FAIL;
...
{
		uint16_t buf=0x1C;
		SD8931_write_addr(0x0E,&buf,1,IIC_x);
...
{	
		uint16_t buf=0x88;
		if(SD8931_write_addr(0x0F,&buf,1,IIC_x)!=RTC_OK)
			return RTC_FAIL;
...
{
		uint16_t buf=0x80;
		SD8931_write_addr(0x0F,&buf,1,IIC_x);
...
返回值：TRUE-成功
**/
uint8_t sd8931_idbuff(uint16_t *Data, i2c_device_bus IIC_x)
{
...
uint8_t sd8931_ReadTemp(i2c_device_bus IIC_x)
{
	...
	Temp1 = RTC_ReadByte(0x11,IIC_x);
	
	if(Temp1 & 0x80)
	{
		Temp1 = (~Temp1+1)&0xFF;
		sd8931_temp = Temp1;
		bcd_to_dec(sd8931_temp);
		sd8931_temp = 256-sd8931_temp;
...
uint8_t sd8931_id_show(i2c_device_bus IIC_x)
{
		...
		if(sd8931_idbuff(ifbuff,IIC_x)!=TRUE)
			return FALSE;
...
uint8_t sd8931_and_std(void)
{
	...
	if(SD8931_ReadTimeDate(&time_buf,IIC_TEST)!=RTC_FAIL)
	{
...
uint8_t sd8931_time_show(void)
{
	...
	if(SD8931_ReadTimeDate(&time_bufstd,IIC_TEST)!=RTC_FAIL)
	{
		...
		sd8931_id_show(IIC_TEST);
		sd8931_ReadTemp(IIC_TEST);

		OLED_ShowNum(36,43,sd8931_temp,2,6);
...
uint8_t sd8931_time_copy(void)
{
	...
	if(SD8931_WriteTimeDate(&copy_buf,IIC_TEST)!=RTC_FAIL)
	{
...
返回值：sd8931_read_addr(addr,data,len,IIC_x)-读取到的数据
**/
uint8_t RTC_ReadRAM_8931(uint8_t addr,uint16_t *data,uint8_t len,i2c_device_bus IIC_x)
{

	return SD8931_read_addr(addr,data,len,IIC_x);
...
返回值：TRUE-成功
**/
uint8_t RTC_WriteRAM_8931(uint8_t addr,uint16_t *data,uint8_t len,i2c_device_bus IIC_x)
{

	 if(SD8931_write_addr(addr,data,len,IIC_x)!=RTC_OK)
		return RTC_NOACK;
...
{
	uint16_t buf=0x82;
	SD8931_write_addr(0x18,&buf,1,IIC_TEST);
