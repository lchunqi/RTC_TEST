...
返回值：RTC_OK-成功
**/
rtc_exit_code SD8200_read_addr(uint8_t reg_addr, uint16_t *read_buf, uint8_t length, i2c_device_bus bus)
{
...
返回值：RTC_OK-成功
**/
rtc_exit_code SD8200_write_addr(uint8_t reg_addr, uint16_t *write_buf, uint8_t length, i2c_device_bus bus)
{
...
返回值：TRUE-成功
**/
uint8_t sd8200_idbuff(uint16_t *Data, i2c_device_bus IIC_x)
{
...
/**
功能：sd8200读取vbat电压和上位机使用
修改：sjm
参数：IIC_x-iic总线
返回值：sd8200_vbat-读取到vbat电压
**/
uint8_t sd8200_VbatQuan(uint16_t *data,i2c_device_bus IIC_x)
{
	uint8_t time = 5;
	...
	while(time--)
	{
		...
		SD8200_read_addr(0x5A,data,2,IIC_x);
			return TRUE;
...
{
	uint16_t buf=0x40;
	if(SD8200_write_addr(0x07,&buf,1,IIC_x)!=RTC_OK)
			return RTC_FAIL;
	
	buf=0x00;
	if(SD8200_write_addr(0x22,&buf,1,IIC_x)!=RTC_OK)
			return RTC_FAIL;
...
//	uint8_t buf=0;
//	
//	SD8200_write_addr(0x00,&buf,1,IIC_TEST);
//}
...
//	uint8_t temp=0x90;
//		
//	if(SD8200_write_addr(0x07,&temp,1,IIC_x)!=RTC_OK)
//		return RTC_FAIL;
//	
//	OLED_ShowImage(0, 0, 128, 64, wave_32768);
...
rtc_exit_code sd8200_f1(i2c_device_bus IIC_x)
{
	...
	buf=0x5E;
	if(SD8200_write_addr(0x20,&buf,1,IIC_x)!=RTC_OK)
			return RTC_FAIL;	
	buf=0xC7;
	if(SD8200_write_addr(0x21,&buf,1,IIC_x)!=RTC_OK)
			return RTC_FAIL;	
...
返回值：sd8200_read_addr(addr,data,len,IIC_x)-读取到的数据
**/
uint8_t RTC_ReadRAM_8200(uint8_t addr,uint16_t *data,uint8_t len,i2c_device_bus IIC_x)
{
...
返回值：TRUE-成功
**/
uint8_t RTC_WriteRAM_8200(uint8_t addr,uint16_t *data,uint8_t len,i2c_device_bus IIC_x)
{
