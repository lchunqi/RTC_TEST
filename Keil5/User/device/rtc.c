...
返回值：RTC_OK-成功
**/
rtc_exit_code RTC_read_addr(uint8_t reg_addr, uint16_t *read_buf, uint8_t length, i2c_device_bus bus)
{
...
返回值：RTC_OK-成功
**/
rtc_exit_code RTC_write_addr(uint8_t reg_addr, uint16_t *write_buf, uint8_t length, i2c_device_bus bus)
{
...
返回值：RTC_OK-成功
**/
rtc_exit_code RTC_ReadTimeDate2201(rtc_time *time, i2c_device_bus IIC_x)
{
	uint16_t buf[7];
...
返回值：-
**/
void sd25xx_charge_on(void)
{
	uint16_t buf=0x82;

	RTC_WriteOn(IIC_STD);

	RTC_write_addr(0x18,&buf,1,IIC_STD);

	RTC_WriteOff(IIC_STD);
}
...
返回值：FALSE-失败
**/
uint8_t RTC_VbatQuan(uint16_t *data,i2c_device_bus IIC_x)
{
	uint8_t time = 5;
	uint16_t buf=0x10;
...
返回值：TRUE-成功
**/
uint8_t rtc_ifbuff(uint16_t *Data, i2c_device_bus IIC_x)
{
...
返回值：RTC_read_addr(addr,data,len,IIC_x)-读取到的数据
**/
uint8_t RTC_ReadRAM(uint8_t addr,uint16_t *data,uint8_t len,i2c_device_bus IIC_x)
{
...
返回值：TRUE-成功
**/
uint8_t RTC_WriteRAM(uint8_t addr,uint16_t *data,uint8_t len,i2c_device_bus IIC_x)
{
	if(RTC_WriteOn(IIC_x)!=RTC_OK)
		return RTC_FAIL;
...
功能：与上位机交互读取id数据
修改：sjm
参数：*data-读取的数据，IIC_x-iic总线
返回值：TRUE-成功
**/
uint8_t RTC_ReadID(uint16_t *data,i2c_device_bus IIC_x)
{
