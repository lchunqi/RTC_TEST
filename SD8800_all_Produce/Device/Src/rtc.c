...
uint8_t rtc_out_4096(void)
{
	...
	int i;
	for (i = 0; i < 5; i++) {
		rtc_write_multi_bytes(RTC_8800_ADDRESS, 0x10, buffer, 2);
		rtc_read_multi_bytes(RTC_8800_ADDRESS, 0x10, readback, 2);
		if (buffer[0] == readback[0] && buffer[1] == readback[1]) {
			break;
		}
		HAL_Delay(5);
	}
...
{	
	...
	int i;
	for (i = 0; i < 5; i++) {
		rtc_write_multi_bytes(RTC_8800_ADDRESS, WAVE1006_REG_USER_RAM_START + 21, p_dat, 2);
		rtc_read_multi_bytes(RTC_8800_ADDRESS, WAVE1006_REG_USER_RAM_START + 21, readback, 2);
		if (p_dat[0] == readback[0] && p_dat[1] == readback[1]) {
			break;
		}
		HAL_Delay(5);
	}
...
{
	uint8_t buf, readback;
	int i;
	uint8_t osc_buf[2] = {0};
	uint8_t osc_back[2] = {0};

	rtc_set_channel(channel, RTC_INIT_1006);
	buf = 0x40;
	for (i = 0; i < 3; i++) {
		HAL_Delay(5);
		drv_wave1006_write_reg(DEV_SD3068, WAVE1006_REG_CTR3, 1, &buf);//如果是有应该先关闭32K输出(默认为关闭SD3068的32K)
		drv_wave1006_read_reg(DEV_SD3068, WAVE1006_REG_CTR3, 1, &readback);
		if (readback & 0x40) { break; }
	}
	if (i >= 3) { return I2C_NO_ACK; }
	//选择为SD3031封装(必须在进测试模式之前选择，否则芯片pad改变)
	drv_wave1006_read_reg(DEV_SD3068, WAVE1006_REG_USER_RAM_START + 20, 1, &buf);//回读ram21
	buf &= ~0x0f;
	buf |= 0x0c;//根据pad配置表，编号12为3031
	for (i = 0; i < 3; i++) {
		drv_wave1006_write_reg(DEV_SD3068, WAVE1006_REG_USER_RAM_START + 20, 1, &buf);//写入部分ram21寄存器
		drv_wave1006_read_reg(DEV_SD3068, WAVE1006_REG_USER_RAM_START + 20, 1, &readback);
		if (buf == readback) { break; }
	}
	if (i >= 3) { return I2C_NO_ACK; }
	...
	drv_wave1006_read_reg(DEV_SD3068, WAVE1006_REG_USER_RAM_START + 19, 1, &buf);
	buf &= ~0x38;   //清0 RuleTempAdj[2:0]      //ADC尺子 暂选5
	buf |=  0x28;
	for (i = 0; i < 3; i++) {
		drv_wave1006_write_reg(DEV_SD3068, WAVE1006_REG_USER_RAM_START + 19, 1, &buf);
		drv_wave1006_read_reg(DEV_SD3068, WAVE1006_REG_USER_RAM_START + 19, 1, &readback);		
		if (buf == readback) {	break; }
		HAL_Delay(1);
	}
	if (i >= 3) { return I2C_NO_ACK; }
	...
	drv_wave1006_read_reg(DEV_SD3068, WAVE1006_REG_USER_RAM_START + 24, 1, &osc_buf[1]);
	osc_buf[0]  = 0xf7;   //ram24
	osc_buf[1] &= ~0x1f;
	osc_buf[1] |= 0x19;		//ram25
	for (i = 0; i < 3; i++) {
		drv_wave1006_write_reg(DEV_SD3068, WAVE1006_REG_USER_RAM_START + 23, 2, osc_buf);
		drv_wave1006_read_reg(DEV_SD3068, WAVE1006_REG_USER_RAM_START + 23, 2, osc_back);	
		if (osc_buf[0] == osc_back[0] && osc_buf[1] == osc_back[1]) {
			break;
		}
		HAL_Delay(5);
	}
...
uint8_t rtc_8800_adc_testmode(uint8_t channel)
{
	...
	HAL_Delay(5);
	for (i = 0; i < 3; i++) {
		drv_wave1006_read_reg(DEV_SD3068, 0xFD, 1, &buf);
		if ((buf >> 3) & 0x01) { break; }
		HAL_Delay(1);
	}
	if (i >= 3) { return I2C_NO_ACK; }
...
uint8_t rtc_1006_exit_testmode(uint8_t channel)
{
	...
	rtc_set_channel(channel, RTC_SD3031);
	HAL_Delay(5);
	if (rtc_read_multi_bytes(RTC_8800_ADDRESS, 0x00, &buf, 1) == I2C_OK) {
		buf = 0x00;
		for (i = 0; i < 3; i++) {
			rtc_set_channel(channel, RTC_SD3031);
			HAL_Delay(5);
			rtc_write_multi_bytes(RTC_8800_ADDRESS, WAVE1006_REG_EXTEND , &buf, 1);
			rtc_set_channel(channel, RTC_INIT_1006);
			if (rtc_read_multi_bytes(RTC_8800_ADDRESS, 0x00, &buf, 1) == I2C_OK) {
		...
		rtc_set_channel(channel, RTC_INIT_1006);//可能原本就没有进测试模式
		HAL_Delay(5);
		if (rtc_read_multi_bytes(RTC_8800_ADDRESS, 0x00, &buf, 1) == I2C_OK) {
			UsbPrintf("exit testmode success\n");
			return I2C_OK;
