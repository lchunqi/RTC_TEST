...
#define CHIP_SECTOR_NUMBER     13//  100 / 8 = 12.5
    ...
    for (k = 0; k < CHIP_SECTOR_NUMBER; k++) {
        ...
        for (i = 0; i < (SECTOR_SIZE / TEST_RESULT_ONE_SIZE); i++) {//每个sector有8个芯片
            switch (mode) {
                ...
                case FLASH_CLEAR_DOT_4://-40度和85度两个点需要把中心电容对应的误差值清零
                    range = i * TEST_RESULT_ONE_SIZE + TEST_CENTER + (mode / 3) * 4;
                    for (j = 0; j < 4; j++) {
                        g_bufferSector[range + j] = 0xff;
                    }
                case FLASH_CLEAR_DOT_1:
                case FLASH_CLEAR_DOT_2:
                case FLASH_CLEAR_DOT_3:
...
void ClearBsnFlash(void)
{
	...
	EraseInitStruct.Sector      = BSN_IN_SECTOR;
...
//记录高低温测试的次数，每做完1个循环算1次，实际执行时在高温85度记录
//1、先读取测试数据，若还没有85度的记录，则记录，否则退出；
//2、计算上一次的时间，低于1个小时的，退出；
//3、先读出BSN号
//4、擦除flash的那个sector
//5、重新写回去BSN号
//6、写入+1的次数和当前时间
void AddOneTestNumberFlash(void)
{
    char buffer[24] = {0};
	uint8_t flag[MAX_CHIP_NUMBER] = {0xff};
    int i;
    int count = 0;
    RealTime time = {0};

    system_get_real_time(&time);
	flash_read_data(TEST_FALG_START_ADDR + FLAG_TEST_DOT * PAGE_SIZE, flag, MAX_CHIP_NUMBER);
    for (i = 0; i < MAX_CHIP_NUMBER; i++) {
        if ((flag[i] & 0x10) == 0) {
            log_printf("本次测试不增加记录次数。\n");
            return;
        }
    }
    flash_read_data(HISTORY_BUFFER_ADDR, (uint8_t*)buffer, BOARD_HISTORY_LENGTH);//25Q28
    if (buffer[0] != 0xff) {
        log_printf("本次测试不增加记录次数。\n");
        return;
    }
    ControlPrintf("message:更新测试次数\n");
    GetStringFromFlash(buffer, BOARD_HISTORY_LENGTH, BOARD_HISTORY_ADDR);
    CharToInt(buffer, 0, &count);
    flash_write_data(HISTORY_BUFFER_ADDR, (uint8_t*)buffer, BOARD_HISTORY_LENGTH);//25Q128
    count++;
    log_printf("board history:%s\n", buffer);
    UsbPrintf("board history:%s\n", buffer);
    memset(buffer, 0, 24);
    GetStringFromFlash(buffer, BSN_LENGTH, BSN_ADDR);

    ClearBsnFlash();
	SaveStringToFlash(buffer, BSN_ADDR);
    memset(buffer, 0, 24);
    snprintf(buffer, 20, "%d %02d/%02d/%02d %02d:%02d", count, time.year % 100, time.mon % 100, time.day % 100, time.hour % 100, time.min % 100);
