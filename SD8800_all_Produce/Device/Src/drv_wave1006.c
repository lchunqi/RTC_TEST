...
wave1006_otp_arg_t g_sd8804_config = {
        ...
        PKG_SD8804,                //package_type_config 注意与PKG_RX8804区别
        ...
        1,                                 //rst_out_support_config //检测复位功能总开关
...
//    pprog = 1
  ...
  drv_wave1006_write_reg(DEV_SD8804, WAVE1006_REG_USER_RAM_START + 1, 1, &buf[0]);
	...
	HAL_GPIO_WritePin(GPIOE, GPIO_PIN_6, GPIO_PIN_SET);//VCE切换
	HAL_GPIO_WritePin(GPIOE, GPIO_PIN_0, GPIO_PIN_SET);//VCE释放
	HAL_Delay(10);
...
uint8_t sd8804_write_otp(uint8_t channel)
{
	...
	g_sd8804_config.temp_offset_config = g_offset;			 //温度偏移值，不同芯片不一样
...
uint8_t sd8800_write_otp(uint8_t channel)
{
	...
	system_get_real_time(&date);
	if (date.year < 24 || date.year > 29 || date.mon < 1 || date.mon > 12 || date.day < 1 || date.day > 31) {//暂时只支持到29年的ID码
		UsbPrintf("date error, check ID error\n");
