...
// #define OFFSET_NO_ROAST	-8     //烧写烘烤125℃一天的芯片
//#define	OTP_LOAD_TEST	1      //直接烧写测试用  烧写板有数据可以直接写入，直接烧写OTP
typedef struct {
	float A;
	float B;
	float X0;
...
float compensate_actuality_left(float nowTemp)//基于20240816测试的数据，计算的一段补偿值，使得5度不要负偏
{
	float result, center;
...
float compensate_actuality_right(float nowTemp)//基于20240816测试的数据，计算的一段补偿值，使得45度不要正偏
{
	float result, center;
	center = g_input[TEST_RTC_TEMP_Z29] + 40;
	result = 5 - 0.003 * (nowTemp - center) * (nowTemp - center);
...
uint8_t calculate_wave1006_temp_cap(uint8_t rtcNum, float* cap_value, int* digtal, uint16_t* offset)
{
	...
	*offset = g_testData.tempConfig;
    ...
    for (i = 0; i < TEMP_ADC_LENGTH; i++) {
		// if (i % 20 == 0) { UsbPrintf("\n[%d]\t", -60 + i / 2); }
		if (adcStart + i < g_input[TEST_RTC_TEMP_F40]) {//低于-40度的数值
