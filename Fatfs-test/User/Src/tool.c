#include "tool.h"

// //精准延时（72MHz）//循环转圈
static void delay_us(uint32_t us)
{
    uint32_t count = us * 8;
    while(count--) {__NOP();}
}


void Soft_UART_Init(void)
{
    GPIO_InitTypeDef GPIO_InitStruct = {0};
    __HAL_RCC_GPIOB_CLK_ENABLE();
    
    // TX初始化
    GPIO_InitStruct.Pin = SOFT_UART_TX_Pin;
    GPIO_InitStruct.Mode = GPIO_MODE_OUTPUT_PP;
    GPIO_InitStruct.Pull = GPIO_PULLUP;
    GPIO_InitStruct.Speed = GPIO_SPEED_FREQ_HIGH;
    HAL_GPIO_Init(SOFT_UART_TX_GPIO_Port, &GPIO_InitStruct);
    
    // RX初始化
    GPIO_InitStruct.Pin = SOFT_UART_RX_Pin;
    GPIO_InitStruct.Mode = GPIO_MODE_INPUT;
    GPIO_InitStruct.Pull = GPIO_PULLUP;
    HAL_GPIO_Init(SOFT_UART_RX_GPIO_Port, &GPIO_InitStruct);
    
    // TX默认高电平
    HAL_GPIO_WritePin(SOFT_UART_TX_GPIO_Port, SOFT_UART_TX_Pin, GPIO_PIN_SET);
}

void Soft_UART_SendByte(uint8_t data)
{
    uint8_t i;
    // 起始位
    HAL_GPIO_WritePin(SOFT_UART_TX_GPIO_Port, SOFT_UART_TX_Pin, GPIO_PIN_RESET);
    delay_us(BAUD_DELAY_US);
    
    // 数据位(低位先行)
    for(i=0; i<8; i++)
    {
        HAL_GPIO_WritePin(SOFT_UART_TX_GPIO_Port, SOFT_UART_TX_Pin,
            (data&1) ? GPIO_PIN_SET : GPIO_PIN_RESET);
        data >>= 1;
        delay_us(BAUD_DELAY_US);
    }
    
    // 停止位
    HAL_GPIO_WritePin(SOFT_UART_TX_GPIO_Port, SOFT_UART_TX_Pin, GPIO_PIN_SET);
    delay_us(BAUD_DELAY_US);
}

void Soft_UART_SendStr(uint8_t *str)
{
    while(*str) 
    {
        Soft_UART_SendByte(*str++);
    }
}

uint8_t Soft_UART_ReceiveByte(uint8_t *data)
{
     // 入参空指针校验
    if(data == NULL) 
        return 0xFF;

    uint32_t timeout = 5000;
    uint32_t tick = 0;

    // 1. 等待起始位（下降沿）
    while (HAL_GPIO_ReadPin(SOFT_UART_RX_GPIO_Port, SOFT_UART_RX_Pin) != GPIO_PIN_RESET)
    {
        if (++tick > timeout) return 0xFF;
        delay_us(1);
    }

    // 2. 关中断，保证时序不被打断
    __disable_irq();

    // 3. 【关键修复】延时 1.5 位周期，对准数据位中心
    delay_us(BAUD_DELAY_US);
    delay_us(BAUD_DELAY_US / 2);

    *data = 0;
    // 4. 采样8位数据（低位先行）
    for (uint8_t i = 0; i < 8; i++)
    {
        if (HAL_GPIO_ReadPin(SOFT_UART_RX_GPIO_Port, SOFT_UART_RX_Pin) == GPIO_PIN_SET)
            *data |= (1 << i);
        delay_us(BAUD_DELAY_US);
    }

    // 5. 开中断
    __enable_irq();

    // 6. 等待停止位
    while (HAL_GPIO_ReadPin(SOFT_UART_RX_GPIO_Port, SOFT_UART_RX_Pin) != GPIO_PIN_SET)
    {
        if (++tick > timeout) return 0xFF;
        delay_us(1);
    }

    return 1;
}


uint8_t Soft_UART_ReceiveString(uint8_t *data)
{
    if (data == NULL) return 0xFF;
    uint8_t i = 0;
    while (1)
    {
       
        if(Soft_UART_ReceiveByte(&data[i]) != 1) 
        {
            return 0xFF;
        }
        if (data[i] == '\r' || data[i] == '\n') 
        {
            data[i] = '\0';
            return i;
        }
        i++;
        
    }

    //HAL_GPIO_TogglePin(LED_PORT, LED_PIN);
}

/*******************************
修复点2：修复延时失效、内存越界、超时无效
*******************************/
// 【修复】增加max_len入参，由外部指定缓冲区长度，彻底杜绝越界
// uint16_t Soft_UART_ReceiveString(uint8_t *data, uint16_t max_len)
// {
//     // 空指针校验
//     if (data == NULL || max_len == 0) 
//         return 0xFF;

//     uint16_t i = 0;
//     uint32_t timeout_cnt = 0;
//     const uint32_t TIMEOUT_MS = 500;  // 超时500ms

//     // 循环接收（严格按外部缓冲区长度限制，安全可靠）
//     while (i < max_len - 1)
//     {
//         uint8_t ch = 0;
//         // 尝试接收单字节
//         if (Soft_UART_ReceiveByte(&ch) == 1)
//         {
//             timeout_cnt = 0; // 收到数据，重置超时

//             // 遇到结束符，终止接收
//             if (ch == '\r' || ch == '\n')
//             {
//                 break;
//             }
//             data[i++] = ch; // 存储有效数据
//         }
//         else
//         {
//             // 【核心修复】用可靠delay_us代替空循环，防止编译器优化
//             delay_us(1000);
//             timeout_cnt++;

//             // 超时退出
//             if (timeout_cnt >= TIMEOUT_MS)
//             {
//                 return 0xFF;
//             }
//         }
//     }

//     data[i] = '\0';    // 添加字符串结束符
//     return i;          // 返回实际接收长度
// }