...
int main(void)
{
  ...
  SystemCoreClockUpdate();//更新获取时钟频率
  ...
  //uint8_t u2_buf[256];//串口2
  //RTC_SetDateTime(2026, 3, 23, 20, 25, 50); 
  
  // uint32_t systemTick = 0;
  // uint8_t year,month, day;
  // uint8_t hh, mm, ss;
  // RTC_GetCurrentDate(&year, &month, &day);
  // UsbPrintf("RTC时间：%d-%d-%d\n", year, month, day);

	UsbPrintf("===== Data:%s Time:%s ! =====\r\n",__DATE__ , __TIME__);

  //UsbPrintf("SystemCoreClock = %d Hz\n", SystemCoreClock);
  //W25Q64_FullTest();  //W25Q64功能测试
  //FatFs_Test();       //fatfs读写测试

  Soft_UART_SendStr((uint8_t*)"666\r\n");
  
  uint16_t IO_len = 0, i=0;
  uint8_t IO_buf[255];
  ...
  while (1)
  {
    ...
    // uint8_t ch = Soft_UART_ReceiveByte(IO_buf);
    // if(ch != 0xFF)
    // {
    //     Soft_UART_SendByte(IO_buf[0]); // 回显
    // }
    
    IO_len = Soft_UART_ReceiveString(IO_buf);
    if( IO_len != 0xFF)
    {
      UsbPrintf("IO 收到：%d\r\n", IO_len);
      while( IO_len > i )
      {
        Soft_UART_SendByte(IO_buf[i++]);
      }
