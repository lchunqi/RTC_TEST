...
// 获取当前日期（基于秒计数器反向解析年/月/日）
// 原来：void RTC_GetCurrentDate(uint16_t *year, uint8_t *month, uint8_t *day)
// 现在：year 改成 uint8_t *
void RTC_GetCurrentDate(uint8_t *year, uint8_t *month, uint8_t *day)
{
    uint32_t total_sec = (RTC->CNTH << 16) | RTC->CNTL;
    uint32_t total_days = total_sec / 86400;
    uint16_t mon_day[] = {31,28,31,30,31,30,31,31,30,31,30,31};
    uint16_t full_year = 2000;  // 内部仍然用完整年份计算
    ...
    *year = (uint8_t)(full_year % 100);
    ...
    for(m=0; m<12; m++)
    {
        if(total_days < mon_day[m])
            break;
        total_days -= mon_day[m];
    }
